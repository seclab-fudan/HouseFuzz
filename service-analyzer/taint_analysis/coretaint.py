...
class CoreTaint:
    ...
    def __init__(
        ...
        :param black_calls: calls to not follow in any case
        :param black_list: black lists of addresses to not follow
        ...
        self._black_list = list(black_list) if black_list else []
    ...
    def add_sum_f(self, addr, f, init=True):
        ...
        try:
            ...
            if (
                ...
                # <=2 instead of ==1 because mips jump block has 2 instructions
    ...
    def get_taint_tags(self, var):
        """
        Get the taint tag of a variable

        :param var: variable
        :return: taint tag
        """
        if not self.is_tainted(var):
            return None
        variables = getattr(var, "variables", None)
        if not variables:
            return None

        tags = []
        for v in variables:
            if v.startswith(self._taint_buf + '_'):
                tag = v[len(self._taint_buf) + 1:]
                tags.append(tag)
        return tags
    ...
    def get_any_addr(self, path):
        """
        Gets the path current address, even it is not active
                    ...
                    if dyn_addr is not None:
                        if dyn_addr in self._summarized_f:
                            ...
                            if dyn_addr in self._imports_by_addr:
                                self._imports_by_addr[addr] = self._imports_by_addr[dyn_addr]
                        else:
    ...
    def _follow_call(self, prev_path, suc_path, args=None):
        ...
        # Default exploration rule: follow if any of the calling registers is tainted
        for reg in calling_regs:
    ...
    def _check_and_record_path_end(self, state, path):
        ...
        if self._sanitized_path:
            self.log.info("vulnerability detected")
    ...
    def _do_check_path(self, path, **kwargs):
        try:
            if self._check_func:
                self._check_func(path, ct=self, **kwargs)
        except:
            self.log.error(
    ...
    def _flat_explore(
        ...
        **kwargs,
        ...
        if self._black_list and current_path_addr in self._black_list:
            self.log.info(f"Blacklisted block at {hex(current_path_addr)}")
        ...
        for next_state in succ_states:
            ...
            try:
                ...
                if (
                    ...
                    next_addr = self.get_any_addr(next_path)
                    if not next_addr:
                        self.mark_path(invalid=True, state=next_state)
                        continue
    ...
    def _merge_ret_records(self, collected_ret_records):
        res_records = []
        # group by address and function call stack
        grouped_ret_records = {}

        for _, state_group in itertools.groupby(
            collected_ret_records.keys(), key=lambda s: (s.addr, tuple(s.info.func))
        ):
            # find common parent
        ...
        for parent_state_history, records in grouped_ret_records.items():
            ...
            get_call_stack = lambda r: tuple(r["state"].info.func)

            group_iter = itertools.groupby(
                records, key=lambda r: (get_ret_addr(r), get_ret_val(r)._hash, get_call_stack(r))
            )
