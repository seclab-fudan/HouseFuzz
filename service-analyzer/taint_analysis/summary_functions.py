...
class SummarizedFunction(abc.ABC):
    ...
    def __call__(self, core, prev_path, succ_path, **kwargs):
        ...
        call_log_msg = (
            f"Call summarized function {self.name} (addr={hex(addr)}) {self.func_name}"
        )
        try:
            self.run(**kwargs)
    ...
    def run(self, **kwargs):
        ...
        for match in re.finditer(pattern, fmt_str):
            ...
            if core._is_reg_or_mem_taint_quick(self.succ_state, va_reg_name):
                reg_val = getattr(self.succ_state.regs, va_reg_name)
                reg_mem_val = self.succ_state.memory.load(reg_val, 4)
                tags = core.get_taint_tags(reg_mem_val)
                if tags:
                    taint_tags.update(tags)
                tags = core.get_taint_tags(reg_val)
                if tags:
                    taint_tags.update(tags)
                tainted = True
        ...
        taint_tags = [tag for tag in taint_tags if tag.startswith('source')]
        if tainted and taint_tags:
            if len(taint_tags) == 1:
                core.apply_taint(path, dest_addr, taint_tags[0], length * 8)
            else:
                core.apply_taint(path, dest_addr, '_'.join(taint_tags), length * 8)
...
class add_taints_for_string(SummarizedFunction):
    def __init__(self, taint_positions=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._taint_positions = list(taint_positions) if taint_positions else None
    ...
    def run(self, **kwargs):
        ...
        # Taint all except constant string pointers
        str_reg_names = set()
        for reg_name in arg_reg_names(p):
            reg_val = getattr(state.regs, reg_name)
            key_str = core.safe_load_str(path, reg_val)
            if key_str:
                str_reg_names.add(reg_name)
        ...
        inferred_reg_names = set()
        for reg_name in core._infer_calling_convention(self.prev_state):
            if reg_name in str_reg_names:
                continue
            inferred_reg_names.add(reg_name)
        inferred_reg_names.add(ret_reg_name(p))

        if self._taint_positions:
            for pos in self._taint_positions:
                if pos < 0:
                    # representing return value
                    reg_names.add(ret_reg_name(p))
                else:
                    reg_names.add(arg_reg_name(p, pos))
        else:
            reg_names = inferred_reg_names

        caller_block = self.prev_state.block()
        caller_block_start = caller_block.addr
        caller_block_end = caller_block.addr + caller_block.size
        caller_block_info = f"{caller_block_start:#x}-{caller_block_end:#x}"
        for reg_name in reg_names:
            if key:
                tag = f"source_@{key}@{caller_block_info}@{reg_name}"
            else:
                tag = f"source_@?@{caller_block_info}@{reg_name}"
            core.apply_taint_to_reg(path, reg_name, tag)
            reg_val = getattr(state.regs, reg_name)
            core.apply_taint(path, reg_val, tag)

        if ret_reg_name(p) in reg_names:
            if key:
                tag = f"source_@{key}@{caller_block_info}@ret"
            else:
                tag = f"source_@?@{caller_block_info}@ret"
            ret_taint = core.get_sym_val(name=tag, bits=p.arch.bits, taint=True)
            core.apply_taint(path, ret_taint, tag)
            self.ret(ret_taint)
        else:
            self.ret()

        self._rt_info["taint_regs"] = reg_names
