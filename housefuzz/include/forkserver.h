...
typedef struct afl_forkserver {
  ...
  u32 domain_fd;                        /* domain socket fd     */
  ...
  FILE *bincov_file;                    /* All binary coverage file         */
...
#define MAX_PROC_CNT 128
  ...
  u32                   main_child[MAX_PROC_CNT];  /* 主进程产生的子进程也需要qemu显式终止*/
  pthread_mutex_t*      main_child_mutex[MAX_PROC_CNT]; /* 终止同时需要去除锁 */
  u32                   main_child_cnt;
