...
struct queue_entry {
  ...
  u32 bitmap_size,                      /* Number of bits set in bitmap     */
      ...
      ipc_distance;                     /* ipc distance for aflmulti        */
  ...
  u8 *nvram_bitmap;                     /* nvram bitmap for nvram fuzzing   */
  int *pair_case;                       /* queue which makes pair with self */
  u32 tc_ref;                           /* Trace bytes ref count            */

  json_t* root;                         /* json root */
  u32 uri_favored;                      /* currently favored uri            */
  int *uri_index;                       /* currently favored uri's index in directory */
  ...
  u8 seed_type;                         /* HouseFuzz: 0: raw, 1: json */

};


#define TYPE_UTILITY 0
#define TYPE_BG 1
// #define MAX_PROC_CNT (128)
struct proc_entry{
  u8* trace_bits;
  u8* virgin_bits;                      /* fuzzing还没有触碰到的内容 */
  u8* crash_bits;                       /* crash触发时覆盖的bits */
  char* prog_name;                      /* 代表进程名称，也可能是子进程，那么就是父进程的名称*/
  char* parent_name;                    /* 进程父进程名称 */
  int type;                             /* TYPE_UTILITY / TYPE_BG */
  int bitmap_changed;                   /* 在proclist_has_new_bits设置的一个变量,表示是否引入新的virgin bits */
  int shmid;                            /* program share memory ID */
  int pid;                              /* program pid*/
  int ppid;                             /* parent pid */
  int is_child;                         /* 是某个进程的子进程*/
  int has_dict;                         /* 已经被添加到字典中或者已经在字典中检查过*/
  unsigned long progid;                           /* program ID, 哈希值，用于execve产生的进程*/
  long unsigned int fork_addr;          /* fork address only for a child process */
  int valid;                            /* 当前shmem是否被其他进程占用。目前只在进程退出时，设置valid=0 */
  int activated;                        /* 在上一轮fuzzing过程中，当前进程是否产生hit。注意子进程在退出时会直接设置activated，其余进程显式设置。每次计算覆盖率时需要读取全部hit的进程coverage map*/
  int activated_times;
  pthread_mutex_t* mutex;               /* 互斥锁，为了正确只能在一次执行完毕之后才读取shmem*/
  int edges_found;                      /* 发现的edges。在afl-fuzz-stats中会更新*/
  int classified;                        /* 如果在classfied_cnt中进入过，设置这个flag*/
};
...
#define SEED_TYPE_RAW 0
#define SEED_TYPE_JSON 1
...
typedef struct afl_state {
  ...
  u8 no_grammar_mode;                   /* No grammar mode Only use multi process feedback */
  u8 favored_seed_type;                 /* Favored seed type                */
  ...
  u8 skip_deterministic,                /* Skip deterministic stages?       */
      ...
      redqueen_activated_times,         /* times housefuzz redqueen has been activated*/
      redqueen_test,                    /* redqueen testing: need to be deleted in the future*/ 
      housefuzz_cfg_only;               /* cfg only mode: disable online tdg generation*/

  char* housefuzz_redqueen_shmem;       /* housefuzz redqueen share memory  */
  
  u64   redqueen_start_time;            /* redqueen start time, because redqueen start cannot be long*/
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u64 total_crashes,                    /* Total number of crashes          */
      ...
      last_type_start_time,             /* start time for last type         */
  ...
  u64 total_bitmap_size,                /* Total bit count for all bitmaps  */
      ...
      total_ipc_distance;               /* total ipc distance count */
  ...
  u32 *json_queue;                      /* queue case to store only json-type input*/
  u32 json_queue_cnt;                   /* json queue entry count           */
  u32 json_queue_maxsize;

  struct queue_entry **top_rated;           /* Top entries for bitmap bytes */

  struct extra_data *extras;            /* Extra tokens to fuzz with        */
  json_t *json_extras;                  /* Extra tokens to fuzz with(in json)        */
  json_t *all_extras;                   /* All extra token to fuzz with     */
  json_t *perfix_extra;
  json_t *name_extra;
  json_t *fulluri_extra;
  json_t* headers_extra;
  json_t* current_fuzz_uri;             /* currently used uri fuzz dict     */ 
  u32                extras_cnt;        /* Total number of tokens read      */
  u32                extras_json_cnt;    /* json extras cnt                  */
  u32                fulluri_extra_cnt;
  u32                perfix_extra_cnt;
  u32                name_extra_cnt;
  u32                headers_extra_cnt;

  // json_t ** protocal_dict;           /* protocal directory: points to json array of protocal seeds */
  json_t * upnp_dict;
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  /* afl-fuzz for multiple program*/
  pid_t               server_pid;
  pid_t               fuzzer_pid;
  int               domain_fd;
  char*             domain_sock;
  struct sockaddr_un domain_server;
  int               domain_child2parent[2]; // 父进程读取child2parent[0], 写入parent2child[1]
  int               domain_parent2child[2]; // 子进程读取parent2child[0], 写入child2parent[1]
  char              msg_buf[0x30]; // afl-fuzz接受
  int               afl_init_proc; // 是否是一开始等待进程的启动阶段
  struct bg_crash_info* bg_crash;           
  int                meets_bug;   
  bool               meets_cmdinj; 
  ...
  /* 多进程测试专属proc_entry */
  struct proc_entry **proc_list;

  /* 指代目前proc list 中的进程数量。注意不代表最大下标。可能存在下标为空的情况。*/
  u32 list_items;
  u32                   utility_timeout; // utility进程超时


  // struct bg_crash_info **bg_crash_list;
  // u32 bg_crash_cnt;
...
#define clear_bug_status(afl) do { \
  (afl)->meets_bug = 0; \
  (afl)->meets_cmdinj = false; \
  (afl)->bg_crash->shmid = -1; \
  (afl)->bg_crash->addr = -1; \
} while (0)
...
pthread_mutex_t* get_named_shared_mutex(int shmid);
...
void init_domain_socket(afl_state_t *afl);
void proclist_add(afl_state_t *afl, int pid, int shmid, char* name);
void wait_connect(afl_state_t *afl);
void afl_states_handle_pipe();
int delete_sockfile();
...
u8 save_if_interesting_json(afl_state_t *afl, void *mem, u32 len, u8 fault, json_t* root);
...
void load_extras_api(afl_state_t *, u8 *);
...
void update_main_dict(afl_state_t *, char*);
void update_main_dict_wrap(afl_state_t* afl);

json_t* load_json_packet(char* fname);
...
u8   common_fuzz_stuff_json(afl_state_t *afl, u8 *out_buf, u32 len, json_t* json_input);
...
u32    select_next_queue_entry_adapt(afl_state_t *afl, u32 max_tries);
...
void   setup_pipe_sig_handler(void); // added
...
json_t *queue_testcase_get_json(afl_state_t *afl, struct queue_entry *q);
...
void queue_testcase_store_mem(afl_state_t *afl, struct queue_entry *q, unsigned char *);

void queue_testcase_store_mem_json(afl_state_t *afl, struct queue_entry *q, json_t* input_json);
