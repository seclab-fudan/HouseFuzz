...
/* mulafl settings*/
int serv_shmem_id = -1;
unsigned int serv_shmem_fd;
u8*          serv_shmem_map;
char*        serv_shmem_file[0x30]; // 共享内存映射的文件名
...
int inform_aflserver(unsigned int serv_shmem_id){
  int sockfd, len;
  ...
  snprintf(send_buf,0x1f, "S|%d|%d|%s", getpid(),serv_shmem_id, gl_argv_0);
  // printf("send_buf: %s\n", send_buf);
...
json_t *target_json_object = NULL; // 目标json对象的指针

// 在osdep.h中include了对应的共享库

extern char* check_symlink_me(char* path);

int parse_json(){
  int fd = open("/guidance.json", O_RDONLY);
  struct stat sb;
  if(fd == -1){
    printf("can't read guidance file\n");
    exit(-1);
  }
  if(fstat(fd, &sb) == -1){
    printf("can't read file state\n");
    exit(-1);
  }

  void *addr = mmap(NULL, sb.st_size, PROT_READ, MAP_SHARED, fd, 0);
  if(addr == MAP_FAILED){
    printf("can't map guidance file\n");
    exit(-1);
  }
  ...
  json_error_t error;
  json_t *root;
  root = json_loadb(addr, size, 0, &error);
  if(!root){
      printf("can't load json file\n");
      return -1;
  }else if(! json_is_object(root)){
      printf("target is not json object!\n");
      return -1;
  }else{

      // json_t *httpd = json_object_get(root, "httpd");

      char* real_path = NULL;
      char* prog_name = NULL;
      
      if(gl_argv_0[0] == '/'){
        real_path = check_symlink_me(gl_argv_0);
      }else{
            real_path = gl_argv_0;
      }

      char* last_slash = strchr(real_path, '/');
      if(last_slash != NULL){
      ...
      target_json_object = json_object_get(root, prog_name);


      if(target_json_object == NULL || !json_is_object(target_json_object)){
          json_decref(target_json_object);
          return -1;
      }
      
      
      // // 根据所需内容进行相应操作
      // json_object_foreach(httpd, key, value) {
...
pthread_mutex_t* acquire_named_shared_mutex(int shmid){
    ...
    if (fd == -1) {
        // 一般而言如果没有找到可能是afl-fuzz那边还没有创建好
        printf("can't find mutex!\n");
        exit(-1);
    }
...
pthread_mutex_t* read_or_create_named_shared_mutex(int shmid) {
...
int inform_aflserver_debug_me2(char* msg)
{
    int sockfd, len;
    struct sockaddr_un addr;
...
int inform_aflserver_debug_fmt_me2(const char* format, ...){
    va_list args;
    char debug_buf[0x30];
    ...
    inform_aflserver_debug_me2(debug_buf);
}

int has_setup = 0;
int global_shmid = 0;
pthread_mutex_t* global_shmem_mutex = NULL;
extern char* gl_argv_0;

void afl_setup(void) {

  // inform_aflserver_debug_me2("[afl_setup] inside afl_setup\n");


  // 一个好的qemuafl需要同时处理
      // 普通单进程fuzzing情况(从环境变量获取SHM_ENV_VAR)
      // 普通单进程产生子进程，产生新的shmemid，回传给afl-server
      // 同级进程产生时，产生新的shmemid，回传给afl-server

  // (事实上，也可以将shmem和afl解耦，可以后面这么做)
  // 主进程：使用环境变量 AFL_MAIN_BIN=1 代表主binary 当主binary产生fork的时候，需要重置子进程的AFL_MAIN_BIN=0
  // 子进程：父子进程会继承共享内存关系，因此子进程需要关闭原先shmdm_id。注意system()实际上就是fork()+execve("/bin/sh -c xxx")这也属于子进程，但是开销会比较大。这里的fork同样是qemu的fork，因此是可以接管的
  // 同级进程：需要设置 AFL_MAIN_BIN=0 在这种情况下，qemuafl负责申请共享内存，并传递给afl-server
  
  // parse_json();

  // inform_aflserver_debug_me2("[afl_setup] after parse_json\n");
  ...
  char *is_main = getenv("AFL_MAIN_BIN");

  if(is_main && strcmp(is_main, "1") == 0 ){
    // 主进程
    // 这里存在BUG！！主进程执行的system()继承AFL_MAIN_BIN环境变量，导致主进程system()的内容覆盖率全部都写回到主进程的map
    // 不知道为什么system()的时候没有清除。现在加上&& serv_shmem_id != 0来确保获取正确的shmid
    ...
    // 主进程的forkserver不需要向AFL注册proclist，AFL自身处理逻辑可以管理共享内存
    // 后面如果统一可以加进去
    ...
    // 映射和afl-fuzz本身共享的内存
    // while(1){printf("[afl_setup] main binary\n");}
    shm_id = atoi(id_str);
    global_shmid = shm_id;
    afl_area_ptr = shmat(shm_id, NULL, 0);
    // printf("[afl_setup] after shmat\n");
    has_setup = 1;

    // FILE* fp = fopen("/tmp/test/test_fuzz/shmid.log", "a+");
    // fprintf(fp, "(main) %s got shmid %d\n", gl_argv_0, global_shmid);
    // fclose(fp);
  ...
  }else if(is_main == NULL || strcmp(is_main, "0") == 0){ 
    // 如果是子进程先fork，之后再execve，不会有这个环境变量(qemu只将guest程序的写入这里)，这种情况下直接读取syscall.c中的exec_shmid并映射内存
    // 同级进程或者子进程，都需要自己产生共享内存并发送给afl-server
    ...
    if(serv_shmem_id == -1){
      // FILE* fp = fopen("/tmp/test/test_fuzz/shmid.log", "a+");
      // fprintf(fp, "(bg) %s got shmid %d\n", gl_argv_0, global_shmid);
      // fclose(fp);
      
      
      // 一般到达这里说明是由'S'产生的
      // 创建共享内存,如果是execve产生的，则会在读取参数的时候就设置这个全局变量，不会进入这里
      // 或许需要想afl-fuzz请求，看是不是已经存在共享内存，如果存在就不shmget了
      serv_shmem_id = shmget(IPC_PRIVATE, MAP_SIZE + 16, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
      global_shmid = serv_shmem_id;
      global_shmem_mutex = read_or_create_named_shared_mutex(serv_shmem_id);
      FILE* bg_shmem_fp = fopen("/bg_shmem_fp.txt", "a+");
      fprintf(bg_shmem_fp, "bg, name: %s, pid: %d, ppid: %d starts, get shmid:%d\n", gl_argv_0, getpid(), getppid(), serv_shmem_id);
      fclose(bg_shmem_fp);
      // inform_aflserver_debug_me2("before 'S' lock");
      pthread_mutex_lock(global_shmem_mutex);
      // inform_aflserver_debug_me2("after 'S' lock");


      sprintf(id_str, "%d", serv_shmem_id);
      if(serv_shmem_id < 0){

        printf("afl_setup: wrong in shmget\n");
        exit(-1);
      }
      ...
      if(afl_area_ptr == (void*)-1 || !afl_area_ptr){
        // 如果没有创建成功，则在当前进程退出之后，删除共享内存，否则不删除
         shmctl(serv_shmem_id, IPC_RMID, NULL);
      }
      
      if (inst_r) afl_area_ptr[0] = 1;

      /* 后台进程或者子进程需要将 serv_shmem_id 发送给afl-server*/
      if(inform_aflserver(serv_shmem_id) !=0 ){
        perror("inform aflserver wrong!\n");
        exit(-1);
      }
    }else{
      // fork或者execve产生的,应该在参数中传递了shmid,因为父进程在结束之前会向aflserver告知shmid
      // inform_aflserver_debug_me2("from execve,1");
      // FILE* execve_fp = fopen("/qemu_init.log", "a+");
      // fprintf(execve_fp, "%d reach afl_setup, 1", getpid());
      assert(serv_shmem_id > 0);
      // FILE* fp = fopen("/tmp/test/test_fuzz/shmid.log", "a+");
      // fprintf(fp, "(child) %s got shmid %d\n", gl_argv_0, global_shmid);
      // fclose(fp);
      // 根据serv_shmem_id设置锁，这个锁应当在qemu中全局可读
      // global_shmem_mutex = read_or_create_named_shared_mutex(serv_shmem_id);
      // if(global_shmem_mutex == NULL){
        // inform_aflserver_debug_fmt_me2("[afl_setup] mutex is NULL\n");
      // }
      // 对内存区域加锁
      // inform_aflserver_debug_fmt_me2("[afl_setup] before execved prog lock, id=%d\n", serv_shmem_id);
      // pthread_mutex_lock(global_shmem_mutex);
      // inform_aflserver_debug_me2("[afl_setup] after execved prog lock\n");

      global_shmid = serv_shmem_id;
      afl_area_ptr = shmat(serv_shmem_id, NULL, 0);
      // inform_aflserver_debug_me2("from execve,2");
      if(afl_area_ptr == (void*)-1 || !afl_area_ptr){
        // 如果没有创建成功，则在当前进程退出之后，删除共享内存，否则不删除
         shmctl(serv_shmem_id, IPC_RMID, NULL);
         printf("%s can't attach serv_shmem_id: %d\n", gl_argv_0, serv_shmem_id);
      // 由于fork没有exit，因此不用传递HIT,直接写入原先fork的子进程map，不清空并将这个整个作为子进程执行的标记
      }
      // fprintf(execve_fp, "%d reach afl_setup, 2, shmid=%d, ptr=%p", getpid(), global_shmid, afl_area_ptr);
      // fclose(execve_fp);
    }
...
void afl_forkserver(CPUState *cpu) {
  ...
  // u32           map_size = 0;
  // inform_aflserver_debug_fmt("pid:%d,at afl_forkserver", getpid());
  ...
  while (1) {
    ...
    if (!child_stopped) {
      ...
      if (!child_pid) {
        ...
        // 为了解决重复发主进程信息的bug，在主进程有设置环境变量AFL_MAIN_BIN
        // FILE* fp = fopen("/AFLplusplus/debug.txt","a+");
        // fprintf(fp, "[forked] pid: %d, AFL_MAIN_BIN is %s\n", getpid(), getenv("AFL_MAIN_BIN"));
        // fclose(fp);
      ...
      // FILE* fp2 = fopen("/AFLplusplus/debug.txt","a+");
      // fprintf(fp2, "[forked] child pid: %d, AFL_MAIN_BIN is %s\n", child_pid, getenv("AFL_MAIN_BIN"));
      // fclose(fp2);
    ...
    // FILE* fp_exec = fopen("debug_exec.txt", "a+");
    // fprintf(fp_exec, "pid: %d before afl_wait_tsl\n", child_pid);
    afl_wait_tsl(cpu, t_fd[0]);
    // fprintf(fp_exec, "pid: %d after afl_wait_tsl\n", child_pid);
    ...
    /* In persistent mode, the child stops itself with SIGSTOP to indicate
       ...
       again. */
    // fprintf(fp_exec, "afl_wait_tsl, pid=%d, stage 2\n", child_pid);
    ...
    // fprintf(fp_exec, "afl_wait_tsl, pid=%d, stage 3\n", child_pid);

    first_run = 0;

    if (write(FORKSRV_FD + 1, &status, 4) != 4) exit(7);
    // fprintf(fp_exec, "afl_wait_tsl, has written to pipe %d, pid=%d, stage 3\n", FORKSRV_FD + 1, child_pid);
    // fclose(fp_exec);
