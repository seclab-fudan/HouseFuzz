...
// if(strstr(gl_argv_0, "ls")){
//     FILE* fp = fopen("shmid.log", "a+");
//     fprintf(fp, "%s write to global_shmid:%d, serv_shmem_id: %d, pid: %d\n", gl_argv_0, global_shmid, serv_shmem_id, getpid());
//     fprintf(fp, "argv0 is %s\n",gl_argv_0);
//     fclose(fp);
// }
...
int inform_aflserver_debug_me(char* msg)
{
    int sockfd, len;
    ...
    strncpy(addr.sun_path, "/tmp/aflfuzz.sock", sizeof(addr.sun_path)-1);
    len = sizeof(addr);
...
target_ulong global_cur_pc = 0;
target_ulong global_before_cur_pc = 0;
bool first_in = true;
extern json_t* target_json_object;
...
static void afl_gen_trace(target_ulong cur_loc) {
  ...
  /* Optimize for cur_loc > afl_end_code, which is the most likely case on
   ...
   global_before_cur_pc = global_cur_pc;
   global_cur_pc = cur_loc;
   
   long int distance = -1;
   char addr_buf[0x10];
   memset(addr_buf, 0, 0x10);
   snprintf(addr_buf, 0x10, "%d", cur_loc);

//    inform_aflserver_debug_me("at afl_gen_trace");
//    inform_aflserver_debug_me(addr_buf);

   if(target_json_object != NULL){
        // 可以计算距离，已经处理为当前二进制
        json_t *value = json_object_get(target_json_object, addr_buf);
        if(value != NULL){
            distance = json_integer_value(value);
        }
   }
   ...
   if(distance != -1){
      //写入MAP_SIZE和MAP_SIZE+8,其中MAP_SIZE是total_distance_count, MAP_SIZE+8是TOTAL_ENTRY_COUNT
      // 看了INC_AFL_AREA源码，只有在X86会优化成汇编操作，那就干脆不写了，反正固件也不是X86的
      // 先只考虑最小距离
      if(first_in == true){
        // 首次进入，直接写入内容
        *(unsigned long int*)(&afl_area_ptr[MAP_SIZE]) = distance;
        first_in = false;
      }
      else if(*(unsigned long int*)(&afl_area_ptr[MAP_SIZE]) > distance){
            *(unsigned long int*)(&afl_area_ptr[MAP_SIZE]) = distance;
      }
      *(unsigned long int*)(&afl_area_ptr[MAP_SIZE+8]) += 1;
   }
...
//         FILE* fp = fopen("/pc.txt","a+");
//         fprintf(fp, "cur_loc: 0x%x\n", cur_loc);
//         fclose(fp);
//    }
...
//   cur_loc &= (MAP_SIZE_MASK - 1); // 这里控制了hash之后的结果一定在MAP_SIZE中
//   if(getenv("CHILD")){
//     cur_loc += MAP_SIZE_MASK; // child location存放在map的另外一部分
//   }

    
    // if(getenv("CHILD")){
    //     fprintf(fp, "CHILD, hashed pc: 0x%llx\n", cur_loc);
    // }else{
    //     fprintf(fp, "PARENT, hashed pc: 0x%llx\n", cur_loc);
    // }
    // fprintf(fp, "MAP_SIZE: 0x%lx\n", MAP_SIZE);
    // fclose(fp);
  ...
  gen_helper_afl_maybe_log(cur_loc_v); // 增加afl count

  // 增加距离信息
  // add_dist_info(cur_loc);
