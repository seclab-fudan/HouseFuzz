...
# endif /* O32 */
        ...
        case EXCP_AdES:
        {
            ...
            // 信息回传给AFL
            // 用global_before_cur_pc的原因: 否则地址将会是0x61616161这种的
            // 为什么calibrate case的SIGSEGV没有被记录，因为calibrate_case直接调用fuzz_run_target，后面没有save_if_interesting来收集覆盖率以及crash信息
            // 主进程的时候不告知 !(getenv(AFL_MAIN_BIN) && !strcmp(getenv("AFL_MAIN_BIN",1 )))
	    if(!getenv("AFL_MAIN_BIN") || (strcmp(getenv("AFL_MAIN_BIN"),"1")))
        {
            // 非主进程
            fprintf(fp, "TARGET_SIGSEGV has generated, in arm cpu_loop.c, shmid:%d, time:%ld, name:%s, pc=%u\n", global_shmid, t, gl_argv_0, global_before_cur_pc);
            inform_aflserver_bug(global_shmid, global_before_cur_pc);
            fclose(fp);
        }else{
            // 主进程
            fprintf(fp, "TARGET_SIGSEGV has generated, in arm cpu_loop.c, shmid:%d, time:%ld, name:main, pc=%u\n", global_shmid, t, global_before_cur_pc);
            fclose(fp);
        }
