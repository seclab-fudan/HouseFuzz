...
bool hackselect = false;
bool hookhack    = false;
bool hookhack_recved = false;
bool hacksleep   = false;
...
int http_fd = -1;
int conn_fd = -1;
struct epoll_event *epoll_event_addr = NULL; // 因为epoll_wait时参数中无法直接获得什么event和http event相关，需要记录对应的地址
struct epoll_event *epoll_conn_event_addr = NULL;
...
void qemu_set_hackselect(const char* arg){
    hackselect = true;
}

void qemu_set_hacksleep(const char* arg){
    hacksleep = true;
}
...
{
    // 为什么这里只收到一个参数，也就是qemu的路径
    // 需要在afl-common.c中加上execve_arg的引号，不然默认情况下是没有引号的
    qemu_execve_path = strdup(path);
}

void qemu_set_execshmid(const char* shmid)
{
    // fork的进程存在serv_shmem_id， 用命令行指定的进程存在serv_shmem_id
    serv_shmem_id = atoi(shmid);
    // global_shmid = atoi(shmid);
    // FILE* fp = fopen("/qemu_shmid.txt", "a+");
    // fprintf(fp, "got serv_shmem_id from cmdline: %d\n", serv_shmem_id);
    // fclose(fp);
}
...
int inform_aflserver_debug_fmt(const char* format, ...){
    ...
    vsnprintf(debug_buf, 0x30, format, args);
    va_end(args);

    inform_aflserver_debug(debug_buf);
}
                          ...
                          abi_ulong efd_addr, abi_ulong target_tv_addr)
{
    ...
    if (target_tv_addr) {
        ...
        // 防止select超时，但是不清楚后台进程为什么没有覆盖率，是这里的原因吗?
        // ts_ptr = NULL;
        if(hackselect){
            struct timeval my_ts;
            my_ts.tv_sec = 0;
            my_ts.tv_usec = 1000 * 1000;
            ts_ptr = &my_ts;
        }else{
            ts_ptr = NULL;
        }
    ...
    if(hookhack && http_fd == -1) {
        ...
        // fake select result:
        // 1. fake connect at port `fake_port`
        for(int i=0; i<n; i++) {
            struct stat statbuf;
    ...
    /* modified on 0722
        ...
        // 主进程接受过请求，可以直接退出?除非是分段请求?
        // 为什么这里要写成exit(0),7.22改成return 0, netgear ex3700
        // 是不是这种情况：在子进程中关闭了fd但是主进程中没有关闭
        // exit(0);
        return 0; // 表示没有文件描述符可读，也不太对，因为可能有别的文件需要读取
    }
    */
    ...
    // inform_aflserver_debug_fmt("[do_select] hit trylock, trylock_result=%d\n", trylock_result);
                         ...
                         abi_long arg4, abi_long arg5, bool ppoll, bool time64)
{
    ...
    // ppoll和select类似，进入ppoll的时候释放锁，表示一次处理结束
    // ppoll返回的时候，表示有新的连接可以用，则获取锁
    ...
    if (nfds) {
        ...
        for (i = 0; i < nfds; i++) {
            ...
            if(hookhack){
                ...
                if((hookhack && http_fd == -1) /*|| (hookhack && http_fd != -1 && epoll_event_addr != NULL)*/) {
                    struct stat statbuf;
                    struct sockaddr_in sin;
                    socklen_t len = sizeof(sin);
                    uint16_t port = 0;
    ...
    if (ppoll) {
        ...
        // inform_aflserver_debug("[do_ppoll] hit unlock\n");
        ...
        if (arg3) {
            ...
            if(hackselect){
                _timeout_ts.tv_sec = 0;
                _timeout_ts.tv_nsec = 50 * 1000000L;
            }else{
                timeout_ts = NULL;
            }
        ...
        // inform_aflserver_debug_fmt("[do_select] hit unlock, trylock_result=%d\n", trylock_result);
        ...
        while (retries < 3) {
            ...
            if (newport <= 0) {
                if (((struct sockaddr*)addr)->sa_family == AF_INET6 || ((struct sockaddr*)addr)->sa_family == AF_INET) {
                    fprintf(bk_stdout, "[qemu] Forcing port %d to %d and retrying...", newport, fake_port);
                    newport = fake_port;
                }
            }
            else {
...
int inform_aflserver_bug(int shmid, target_ulong bug_addr){
    ...
    snprintf(send_buf,0x20, "B|%d|%u", shmid,bug_addr);
    // printf("%s\n", send_buf);
    // printf("send_buf: %s\n", send_buf);
    // FILE* crash_fp = fopen("/qemu_crash.log", "a+");
    // fprintf(crash_fp, "%s, time: %lu\n", send_buf, time(NULL));
    // fclose(crash_fp);
...
int inform_aflserver_cmdinj(int shmid, target_ulong bug_addr){
    ...
    // 发送 S|<pid>|<shmem_id> 表示pid对应的进程注册了shmem_id对应的共享内存
    snprintf(send_buf,0x20, "I|%d|%u", shmid,bug_addr);
    // printf("%s\n", send_buf);
    // printf("send_buf: %s\n", send_buf);
...
int inform_aflserver_hit(int shmid){
    ...
    memset(request_buf, '\0', 0x20);
    snprintf(request_buf, 0x20, "H|%d", shmid);
    if((sockfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1){
    perror("inform_aflserver_hit: socket error");
    return -1;
  }
    ...
    len = sizeof(addr);
    if (connect(sockfd, (struct sockaddr *)&addr, len) == -1) {
            perror("inform_aflserver_hit: connect");
            return -1;
    }
    if (send(sockfd, request_buf, strlen(request_buf), 0) == -1) {
            perror("inform_aflserver_hit: send");
            return -1;
        }
    close(sockfd);
    return 0;
                           ...
                           abi_ulong target_addrlen_addr, int flags)
{
    ...
    abi_long ret;
    int host_flags;
    // inform_aflserver_debug("do accept, 0\n");

    // char tmp_debug[0x30];
    // memset(tmp_debug, 0 , 0x30);
    // snpprintf(tmp_debug, 0x30, "pid=%d,start accept\n", getpid());

    // inform_aflserver_debug(tmp_debug);

    // inform_aflserver_debug_fmt("pid=%d,start accept\n", getpid());
...
out:
    ...
    // 想明白了，到达accept的时候不能表示一次结束，只能表示下一次开始，但是是可以记录hit的
    ...
    // inform_aflserver_debug("do accept, 3\n");
    host_flags = target_to_host_bitmask(flags, fcntl_flags_tbl);

    if (target_addr == 0) {
        // inform_aflserver_debug("target addr==0\n");
    ...
    // 如果确实存在accept，在后面代码中设置ipc_flag = 1;
    if (!is_error(ret)) {
    ...
    // inform_aflserver_debug("before hit trylock\n");
    // int trylock_result = pthread_mutex_trylock(global_shmem_mutex);
    // inform_aflserver_debug_fmt("after hit trylock, result=%d, return fd=%d\n", trylock_result, ret);
    return ret;
                               ...
                               abi_ulong target_addrlen_addr)
{
    ...
    if(hookhack && conn_fd != -1 && fd == conn_fd) {
        ...
        saddr.sin_family = AF_INET;
        saddr.sin_port = htons(fake_port);
        inet_pton(AF_INET, "127.0.0.1", &saddr.sin_addr);
        host_to_target_sockaddr(target_addr, (void *)&saddr, addrlen);
        if (put_user_u32(addrlen, target_addrlen_addr)) return -TARGET_EFAULT;
        return 0;
...
static abi_long do_epoll_wait(int epfd, struct epoll_event *events,
                      int maxevents, int timeout)
{
    fprintf(bk_stdout,"[do_epoll_wait] hookhack=%d, httpd_fd=%d\n", hookhack, http_fd);
    if(hookhack && http_fd != -1){
        // 之前ctl的时候已经包含了http socket
        // fprintf(bk_stdout, "[HOOK] do_epoll_wait invoked\n");
        if(conn_fd == -1){
            events[0] = *epoll_event_addr;
            events[0].events |= EPOLLIN;
        }else if(conn_fd != -1){
            events[0] = *epoll_conn_event_addr;
            events[0].events |= EPOLLIN;
        }
    // 返回1代表只有一个套接字起作用
    return 1;
    ...
    return get_errno(safe_epoll_pwait(epfd, events, maxevents, timeout,
                                             NULL, 0));
}

static abi_long do_epoll_pwait(int epfd, struct epoll_event *events,
                      int maxevents, int timeout,const sigset_t *sigmask,size_t sigsetsize)
{
    fprintf(bk_stdout,"[do_epoll_pwait] hookhack=%d, httpd_fd=%d\n", hookhack, http_fd);
    if(hookhack && http_fd != -1){
        // 之前ctl的时候已经包含了http socket
        // fprintf(bk_stdout, "[HOOK] do_epoll_wait invoked\n");
        if(conn_fd == -1){
                            ...
                            abi_ulong target_addrlen)
{
    ...
    if (!host_msg)
        return -TARGET_EFAULT;
    if(hookhack && conn_fd != -1 && fd == conn_fd) {
        if(hookhack_recved == true){
            return -1; // netgear router reads socket until return -1
        }
                                 ...
                                 int shmid, abi_ulong shmaddr, int shmflg)
{
    ...
    fprintf(bk_stdout, "[qemu_shmat] shmid:%d\n", shmid);
    ...
    fprintf(bk_stdout, "[qemu_shmat], 1\n");
    ...
    fprintf(bk_stdout, "[qemu_shmat], 2\n");
    ...
    fprintf(bk_stdout, "[qemu_shmat], 3, host_raddr=%p\n", host_raddr);
    ...
    fprintf(bk_stdout, "[qemu_shmat], 4, raddr=%p\n", raddr);
    ...
    fprintf(bk_stdout, "[qemu_shmat], 5, return=%p\n", raddr);
    return raddr;
...
static int ask_aflserver_shmem(pid_t pid, char* name){
    // printf("enter ask_aflserver_shmem\n");
    int sockfd, len;
    struct sockaddr_un addr;
    /* 后面希望 afl-server 能也维护一份进程信息的副本，这样就不需要告知afl了。现在直接转发请求 */
    char request_buf[0x30];
    ...
    /* 如果存在共享内存，返回共享内存id，否则返回-1 */
    memset(request_buf, '\0', 0x30);
    snprintf(request_buf, 0x30, "R|%u|%s", pid, name);
    ...
    len = sizeof(addr);
    if (connect(sockfd, (struct sockaddr *)&addr, len) == -1) {
            perror("ask_aflserver_shmem: connect");
            return -1;
    }
    if (send(sockfd, request_buf, strlen(request_buf), 0) == -1) {
            perror("ask_aflserver_shmem: send");
            return -1;
        }
    
    if(recv(sockfd, reply_buf, 0x30, 0) == -1){
...
static int ask_aflserver_shmem_nonblock(pid_t pid, const char* name) {  
    int sockfd, len;  
    struct sockaddr_un addr;  
    struct timeval tv;  
    char request_buf[0x30];  
...
static int inform_aflserver_child(int pid, int shmid, char* name){
    ...
    struct sockaddr_un addr;
    char request_buf[0x20];
    snprintf(request_buf, 0x20, "C|%d|%d|%s", pid, shmid, name);
    if((sockfd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1){
    perror("inform_aflserver_child: socket error");
    return -1;
  }
    ...
    len = sizeof(addr);
    if (connect(sockfd, (struct sockaddr *)&addr, len) == -1) {
            perror("inform_aflserver_child: connect");
            return -1;
    }
    if (send(sockfd, request_buf, strlen(request_buf), 0) == -1) {
            perror("inform_aflserver_child: send");
            return -1;
        }
    close(sockfd);
    return 0;
...
pthread_mutex_t* create_named_shared_mutex(int shmid) {

    char* filename = (char*)malloc(0x10);
    memset(filename, 0, 0x10);
    snprintf(filename, 0x10, "/%d_mutex", shmid);
...
extern target_ulong global_before_cur_pc;
extern target_ulong global_cur_pc;
                   ...
                   abi_ulong child_tidptr)
{
    ...
    if (flags & CLONE_VM) {
        //fprintf(fp,"[FORK] inside fork, flags & CLONE_VM\n");
        ...
        //fprintf(fp,"[FORK] inside fork, else\n");
        // system 也会执行这里
        ...
        if (ret == 0) {
            ...
            /* 创建新进程映射，并告知afl server*/
            // char *addenv = "CHILD=1";
            // putenv(addenv);
            
            // fprintf(fp,"[FORK] inside fork, ret=0\n");
            // char *p;
            // if((p = getenv("CHILD"))){
            //     fprintf(fp, "[FORK] set environ CHILD=1\n");
            // }
            ...
            // printf("after fork1.5\n");

            if (flags & CLONE_CHILD_SETTID)
                put_user_u32(sys_gettid(), child_tidptr);
            // printf("after fork1.6\n");
            if (flags & CLONE_PARENT_SETTID)
                put_user_u32(sys_gettid(), parent_tidptr);
            // printf("after fork1.7\n");
            ts = (TaskState *)cpu->opaque;
            if (flags & CLONE_SETTLS)
                cpu_set_tls (env, newtls);
            // printf("after fork1.8\n");
            if (flags & CLONE_CHILD_CLEARTID)
                ts->child_tidptr = child_tidptr;


            // if(strcmp(getenv("AFL_MAIN_BIN"),"1") == 0){
            //     // 如果进入这个判断，说明是forkserver所fork出来的，如果没有进入，说明是binary自己fork的
            //     setenv("AFL_MAIN_BIN","0",1);
            //     // 设置AFL_MAIN_BIN=0，标识fork出来的需要告知forkserver
            //     goto fork_custom_end;
            // }
            // FILE* fp = fopen("/AFLplusplus/debug.txt","a+");
            // fprintf(fp, "[fork] pid: %d, ppid: %d, AFL_MAIN_BIN is %s\n", getpid(), getppid(), getenv("AFL_MAIN_BIN"));
            // fprintf(fp, "[fork] pid: %d, ppid: %d, AFL_MAIN_PARENT is %s\n", getpid(), getppid(), getenv("AFL_MAIN_PARENT"));
            // fclose(fp);
            // 用父进程标识?

            // 哦卧槽我知道了，不是程序中的代码不经过qemu的fork，只有程序中的才经过，那其实forkserver产生的fork不经过这里。所以只要全部发就可以了
            // if(getenv("AFL_MAIN_BIN")){
            //     while(1){;} // 被阻塞在这里的应该是fork server产生的。
            //     unsetenv("AFL_MAIN_BIN");
            //     setenv("AFL_MAIN_PARENT","1",1);
            //     goto fork_custom_end;
            // }else{
            //     setenv("AFL_MAIN_PARENT","0",1);
            // }


            // 子进程依然是qemu-user，但是他写入MAP的内存位置会有变化，需要告知自身。通过环境变量
            /* 清除原先父进程的内存映射 */
            // 3.8修改 unsetenv("AFL_MAIN_BIN"); 为setenv("AFL_MAIN_BIN","0",1); 不知道会不会有问题
            // unsetenv("AFL_MAIN_BIN");

            // 进行这个判断，因为如果是主进程，并且在afl-setup之前产生fork，不应该新建共享内存
            // 若新建共享内存，则fuzzer端主进程的map将始终未0.
            // 因此，当已经setup之后, 设置为0
            // printf("after fork1.9, has_setup=%d\n", has_setup);
            // 设置锁为NULL，之后后台进程的主进程需要锁

            global_shmem_mutex = NULL;
            if(has_setup == 1){
                setenv("AFL_MAIN_BIN","0",1);
            ...
            // 但是fork的时候可以调用shmdt
            if(shmdt(afl_area_ptr)!=0){
                perror("[FORK] child process shmdt error ");
            }

            /* 创建新的映射，并告知fuzz-server */
            /* 向afl server询问是否已经创建过，怎样标志创建过？？*/
            /* 或许可以通过translate-all.c中afl_gen_trace()的地址作为判断依据*/
            /* global_cur_pc 记录了执行fork前的翻译块pc地址，用这个标志是哪一个fork */

            // 在询问之前，检验是afl-forkserver产生的fork吗,如果是就不发送信息到afl-server
            
            // 如果还没有setup，似乎不需要申请新的内存
            // gl_argv_0
            // printf("after fork2\n");
            // printf("after fork 2.1, gl_argv is %s\n", gl_argv_0);
            serv_shmem_id = ask_aflserver_shmem_nonblock(getpid(), gl_argv_0); // R
            // printf("after for3\n");
            // printf("getppid returns %u\n", getppid());
            // while(1);
            // char debug_tmp[0x30];
            // memset(debug_tmp, 0, 0x30);
            // snprintf(debug_tmp, 0x30, "recv_shmid:%d\n", serv_shmem_id);
            // inform_aflserver_debug(debug_tmp);

            // inform_aflserver_debug_fmt("recv_shmid:%d\n", serv_shmem_id);

            if(serv_shmem_id != -1){
                /* 已经创建过对应子进程 */
                /* 直接使用之前的映射，afl那边直接标记这一块内存可以直接用，不需要重新告诉 */
                // 后面子进程写入的地址就是serv_shmem_id，可以直接shmat
                // printf("fork1, serv_shmem_id=%d\n", serv_shmem_id);
                // global_shmem_mutex = read_or_create_named_shared_mutex(serv_shmem_id);
                // // printf("fork2\n");
                // inform_aflserver_debug_fmt("fork1, before lock %d\n",serv_shmem_id);
                // pthread_mutex_lock(global_shmem_mutex);
                // inform_aflserver_debug_fmt("fork1, after lock %d\n",serv_shmem_id);
                // printf("fork3\n");
                afl_area_ptr = shmat(serv_shmem_id, NULL, 0);
                if(afl_area_ptr == (void*)-1 || !afl_area_ptr){
                    shmctl(serv_shmem_id, IPC_RMID, NULL);
                }
            }else{
                /* 首次创建子进程，较为少见 */
                char* id_str = (char*)malloc(0x20);
                serv_shmem_id = shmget(IPC_PRIVATE, MAP_SIZE+16, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
                if(serv_shmem_id < 0){
                    printf("[FORK] child wrong in shmget\n");
                    exit(-1);
                }
                // printf("fork,0\n");
                // global_shmem_mutex = read_or_create_named_shared_mutex(serv_shmem_id);
                // inform_aflserver_debug_fmt("fork2, before lock %d\n",serv_shmem_id );
                // printf("fork,1\n");
                // pthread_mutex_lock(global_shmem_mutex);
                // printf("fork,2\n");
                // inform_aflserver_debug_fmt("fork2, after lock %d\n",serv_shmem_id );
                

                sprintf(id_str, "%d", serv_shmem_id);
                afl_area_ptr = shmat(serv_shmem_id, NULL, 0);
                if(afl_area_ptr == (void*)-1 || !afl_area_ptr){
                    // 如果映射失败?
                    // inform_aflserver_debug_fmt("shmat failed for pid %d", getpid());
                    shmctl(serv_shmem_id, IPC_RMID, NULL);
                }

                // 在没有找到子进程的情况下，使用特定格式告知afl-server产生新的子进程
                if(inform_aflserver_child(getpid(), serv_shmem_id, gl_argv_0) !=0 ){
                    perror("inform aflserver_child wrong!\n");
                    exit(-1);
                }
                
            }
        }else{
            // 如果 has_setup == 0, 也就是没有到forkserver的地方，那么直接读取原先的map并写入
            // char debug_tmp[0x30];
            // memset(debug_tmp, 0, 0x30);
            // snprintf(debug_tmp, 0x30, "%s fork, not setup\n", gl_argv_0);
            // inform_aflserver_debug(debug_tmp);
            // inform_aflserver_debug_fmt()
        }
        } else {
...
* @return int 1: 复制成功；-1: 复制失败
**/
static int copyFile(char *fileRead, char *fileWrite){
    ...
    if( (fpWrite=fopen(fileWrite, "wb")) == NULL ){
        free(buffer);
        printf("Cannot open  file write %s, result=%d\n", fileRead,fpRead, fileWrite,fpWrite);
        return -2;
    }
...
int has_file_under(const char* dir_prefix, const char* redirected_path){
    // 判断redirected_path是否出现在afltmp下
    printf("try to find %s under %s..\n", redirected_path, dir_prefix);
    DIR* dir;
    struct dirent *ent;

    // 打开文件夹
    dir = opendir(dir_prefix);
    ...
    while((ent = readdir(dir)) != NULL){
        // 过滤.和..
        if(strcmp(ent->d_name, redirected_path) == 0){
            closedir(dir);
            printf("housefuzz found %s under %s\n", redirected_path, dir_prefix);
            return 1;
        }
    }
    closedir(dir);
...
static int do_access(const char *pathname, int mode){
    ...
    if(strstr(pathname, "/lib/") ||
       strstr(pathname, "/proc") ||
        ...
        !strcmp(pathname,"/nvram_shm_cow") || 
        !strcmp(pathname,"/nvram.log") || 
        strstr(pathname, "redqueen")){
            goto resume_access;
    }
    ...
    char* replaced_str = replace_slash_with_underscore(pathname);
    snprintf(redirected_path, PATH_MAX, "/afltmp/%s", replaced_str);
    free(replaced_str);
    
    // fprintf(bk_stdout, "do_access redirected %s to %s\n", pathname, redirected_path);
    pathname = redirected_path;
...
static int do_openat(void *cpu_env, int dirfd, const char *pathname, int flags, mode_t mode)
{
    ...
    char* changed_rpath = NULL;
    char redirected_path[PATH_MAX+1];
    char fix_redirected_path[PATH_MAX+1];
    memset(redirected_path, 0, PATH_MAX+1);
    int hackproc_changed = 0;

    if (hackproc) {
        // inform_aflserver_debug_fmt("o:pn:%s",pathname);
        result = realpath(pathname, rpath);
        if (result == NULL){
            snprintf(rpath, PATH_MAX, "%s", pathname);
            // inform_aflserver_debug_fmt("o:rn:NULL");
        }
        // inform_aflserver_debug_fmt("o:rn:%s", rpath);
    ...
    // inform_aflserver_debug_fmt("o:f:%s",pathname);

    // 防止固件修改文件系统，将所有对文件写入的操作重定向到/afltmp文件夹下
    
    if(hackproc_changed != 1){
        if(strstr(pathname, "/lib/") || 
         strstr(pathname, "/proc") ||
        !strcmp(pathname, "/nvram_cow_lock") || 
        !strcmp(pathname, "/nvram_lock") || 
        ...
        memset(fix_redirected_path, 0, PATH_MAX+1);
        result = realpath(pathname, rpath);
        // 如果real_path出错将返回NULL
        if (result == NULL){
            snprintf(rpath, PATH_MAX, "%s", pathname);
        }

        printf("redirected path is %s\n", rpath);
        changed_rpath = replace_slash_with_underscore(rpath);
        
        
        if(has_file_under("/afltmp", changed_rpath)){
            snprintf(redirected_path, PATH_MAX+1, "/afltmp/%s", changed_rpath);
            pathname = redirected_path;
            free(changed_rpath);
            goto resume_open;
        }
        if(has_file_under("/aflfix", changed_rpath)){
            // 要找的文件是初始化过程中创建的
            // 为了防止修改fix文件，将这个文件移动到/afltmp
            snprintf(fix_redirected_path, PATH_MAX+1, "/aflfix/%s", changed_rpath);
            snprintf(redirected_path, PATH_MAX+1, "/afltmp/%s", changed_rpath);
            copyFile(fix_redirected_path, redirected_path);
            free(changed_rpath);
            pathname = redirected_path;
            goto resume_open;
        }

        // 没有对应的文件才从原先文件系统中复制。如果有文件在/afltmp下面说明之前已经从文件系统或者aflfix中复制过
        if(flags & O_WRONLY || flags & O_RDWR){
            // 对文件存在写入操作，先复制文件到/afltmp，之后把文件名称修改一下
            // 并且记录之前已经修改的文件。如果要读取的文件时之前修改之后的，虽然是用读取的方法，但是还是重定向
            // FILE* move_fp = fopen("/move_file.txt", "a+");
            snprintf(redirected_path, PATH_MAX+1, "/afltmp/%s", changed_rpath);
            // printf("moved file: %s\n", redirected_path);
            free(changed_rpath);

            // 如果不是之前修改过的，先copy原先文件到afltmp，再打开
            // 有时原始文件可能不存在，例如直接以打开目标新文件并写入，如果原始文件不存在，copyFile也会正常返回
            copyFile(rpath, redirected_path);
            pathname = redirected_path; 
        }
            // fclose(move_fp); 
    }
...
resume_open:   
    // fprintf(bk_stdout, "result: open: %s\n",path(pathname));
...
static char* new_check_symlink(char *path) {
    char *result = (char*) malloc(MAX_PATH_LENGTH);
    if (result == NULL) {
        // inform_aflserver_debug("Out of memory\n");
        exit(1);
    ...
    ssize_t len = readlink(path, temp, MAX_PATH_LENGTH - 1);
    if (len != -1) {
        temp[len] = '\0';
    ...
    /* Don't forget to free temp when you're finished with it. */
    free(temp);
}


static char* check_symlink(char *path) {
    char *result = (char*) malloc(MAX_PATH_LENGTH);
    if (result == NULL) {
    ...
    ssize_t len = readlink(path, result, MAX_PATH_LENGTH - 1);
    if (len != -1) {
        result[len] = '\0';
        return result;
    } else {
...
char* my_dirname(char *path) {
    ...
    if (last_slash == path) {
        ...
        /* Terminate the string on the last slash. */
        *last_slash = '\0';
    }
    return result;
}
...
char* check_symlink_me(const char* path){

    char *link_target = (char*)malloc(0x50);
    char *absolute_path= (char*)malloc(0xa0);
    char *dir;
    char* real_start = NULL;
    ssize_t len;
    // inform_aflserver_debug("0");

    if ((len = readlink(path, link_target, 0x50)) != -1) {
    link_target[len] = '\0'; // 确保字符串以null结尾
    // inform_aflserver_debug("1");
    // 判断是不是/开头
    if(link_target[0] == '/'){
        free(absolute_path);
        return link_target;
    }else{
        // inform_aflserver_debug("2");
        // 相对路径，获取当前目录
        if(link_target[0] == '.' && link_target[1] == '/'){
            real_start = link_target + 2;
        }else{
            real_start = link_target;
        }
        dir = my_dirname(path); // 不会是没有这个吧。。。？
        // inform_aflserver_debug("2.5");

        // 组装
        snprintf(absolute_path, 0xa0, "%s/%s", dir, real_start);
        free(link_target);
        // inform_aflserver_debug(absolute_path);
        return absolute_path;
        
    }
    } else {
        // inform_aflserver_debug("3");
        // 这种情况下，目标文件不是软连接，直接返回原先的path即可
        free(link_target);
        free(absolute_path);
        return strdup(path);
    }
...
// 使用手写的crc32函数实现ELF文件哈希

#define MAX_BUF_FILE 8192
uint32_t crc_table[256];
int crc_table_gened = 0;
...
uint32_t calc_crc32_of_file(const char *filename) {
    // inform_aflserver_debug("0");
    if(!crc_table_gened){
        gen_crc32_table();
    }
    // inform_aflserver_debug("0.5");
    ...
    char buf[MAX_BUF_FILE];
    ssize_t len;
    uint32_t crc = 0xFFFFFFFF;

    while ((len = read(file, buf, MAX_BUF_FILE)) > 0) {
        for (ssize_t i = 0; i < len; ++i)
            crc = crc_table[(crc ^ (uint8_t)buf[i]) & 0xFF] ^ (crc >> 8);
    }
    // inform_aflserver_debug("2");
...
static int inform_aflserver_exec(uint32_t hashid, int shmid, char* name)
{
    ...
    /* 如果存在共享内存，返回共享内存id，否则返回-1 */
    memset(request_buf, '\0', 0x30);
    snprintf(request_buf, 0x30, "X|%u|%d|%s", hashid, shmid, name);
...
int my_small_strncmp(const char *s1, const char *s2, size_t len) {
...
void remove_env_var(char *envp[], const char *target) {
    int i, j;
    size_t target_len = strlen(target);

    for (i = 0; envp[i] != NULL; ++i) {
        if (my_small_strncmp(envp[i], target, target_len) == 0 && envp[i][target_len] == '=') {
            // 找到目标环境变量，进行删除操作
            // 根据实际情况决定是否需要 free 操作
            // free(envp[i]);
...
void debug_print_arg_env(char *argv[], char *envp[]){
    FILE* fp = fopen("/qemu_execve.log", "a+");
    fprintf(fp, "inside debug_print_arg_env\n");
    for(int i = 0; ; i++){
        char* arg = argv[i];
        if(arg == NULL){
            break;
                  ...
                  char *envp[])
{
    ...
    char **new_argp;
    // offset=3 前三位放 qemu_path, -0, prog_name
    ...
    int execved_shmid;
    int cmdi_score = 0;
    // FILE* exec_fp = fopen("/qemu_exec.log", "a+");
    fprintf(bk_stdout, "[qemu] doing qemu_execven on filename %s\n", filename);
    // fprintf(exec_fp, "[qemu] doing qemu_execven on filename %s\n", filename);
    // 关闭翻译管道,当前关闭的时候应该已经在目标进程中了(由fork产生)，目标进程和翻译管道对应的进程和主进程应该不是同一个
    close(TSL_FD);

    // 进程对象是filename而不是-0的。但是有时-0只是一个binary的一个功能部分，因此一开始认为不能用-0作为分割，而是用的filename绝对路径的hash
    // 但是针对脚本，filename是xxx.sh，却不应该将这部分记录在xxx.sh中，而应该记录在-0的/bin/sh中。这样能够减少管理成本
    ...
    // execve的需要有一个新的map。分为几个步骤
    // 1. (判断目标程序是不是符号链接)，获取目标程序哈希值，用哈希值作为id
    // 2. 发送X|<progID>|<shmid> 告知afl-server
    // 3. 后续不能和子进程一样使用pid作为查询依据，需要新写一个查询函数，例如 Q|<progID>

    // 向afl-server发送的逻辑可以直接在父进程中处理，之后父进程传输一个id作为参数，后续读取即可
    ...
    // memset(debug_tmp, 0, 0x30);
    // snprintf(debug_tmp, 0x30, "exec:%s\n", filename);
    // inform_aflserver_debug(debug_tmp);
    // inform_aflserver_debug_fmt("exec:%s\n", filename);

    char* real_path = check_symlink_me(filename);
    fprintf(bk_stdout, "[qemu_exeve] real_path %s\n", real_path);

    

    // memset(debug_tmp, 0, 0x30);
    // if(real_path){
    //     snprintf(debug_tmp, 0x30, "syml_r:%s", real_path);
    // }else{
    //     snprintf(debug_tmp, 0x30, "syml_r:null");
    // }
    ...
    // inform_aflserver_debug("before crc");
    ...
    {
        // inform_aflserver_debug("not found");
        
        // 一次找不到文件时，可以在四个目录下都寻找，如果都找不到，可以直接报告出现命令注入
        ...
        const char* path1 = "/usr/local/sbin/";
        const char* path2 = "/usr/local/bin/";
        const char* path3 = "/usr/sbin/";
        const char* path4 = "/usr/bin/";
        const char* path5 = "/sbin/";
        const char* path6 = "/bin/";
        const char *path_case[] = {path1, path2, path3, path4, path5, path6};

        char* new_path = (char*)malloc(0x60);
        memset(new_path, 0, 0x60);
        for(int path_cnt = 0; path_cnt < 6; path_cnt++){
            strcat(new_path, path_case[path_cnt]);
            strcat(new_path, prog_name);
            if(access(new_path, F_OK) != 0){
                // 未找到
                memset(new_path, 0, 0x60);
                continue;
            }else{
                // 找到目标文件，直接替换real_path到目标路径
                free(real_path);
                real_path = new_path;
                goto qemu_execve_cont;
            }
        }
        

        // 到达这里说明没找到，可以报告aflserver出现了bug
        // inform_aflserver_cmdinj(global_shmid, global_cur_pc);
        
        // 如果在上述路径都没找到，说明可能是crash
        ...
        fprintf(bk_stdout, "[qemu_execve] cannot find real_path %s\n", real_path);
        return -1; // 如果找不到目标文件，提前终止，并且或许是一个crash？不一定，sh执行命令的时候会从多个路径下面寻找
...
qemu_execve_cont:
    // FILE* exec_fp = fopen("cmdlist.txt", "a+");
    // fprintf(exec_fp, "exec: %s, cmd: %s\n",real_path, cmd_buf);
    // fprintf(bk_stdout, "[qemu] after checking, real_path=%s, cmd_buf = %s\n", filename);
    // fclose(exec_fp);
    
    // 子进程全局锁应该为空，由httpd等待其结束
    // global_shmem_mutex = NULL;
    ...
    fprintf(bk_stdout, "[qemu_exeve] reach 1 \n");
    // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d\n", filename ,execved_shmid, getpid());
    ...
    if ((buf[0] == '#') && (buf[1] == '!')) {
        ...
        *cp = '\0';
        ...
        i_name = cp; // interpreter name
        i_arg = NULL; // interpreter argument
        for ( ; *cp && (*cp != ' ') && (*cp != '\t'); cp++) {
            /* nothing */ ;
        }
        while ((*cp == ' ') || (*cp == '\t')) {
        ...
        if (*cp) { // 如果解释器后面还有内容，则将后面的内容作为解释器的参数
            i_arg = cp;
        }
    ...
    if(execved_shmid == -1){
        ...
        // Prost! New finding !
        // 向afl-server发送新创建shmid的请求
        /* 首次创建子进程，较为少见 */
        fprintf(bk_stdout, "[qemu_exeve] first execute %s\n", real_path);
        execved_shmid = shmget(IPC_PRIVATE, MAP_SIZE + 16, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
        if(execved_shmid < 0){
            printf("[EXECVE] wrong in shmget\n");
            // inform_aflserver_debug("[EXECVE] wrong in shmget\n");
            exit(-1);
        }
        // read_or_create_named_shared_mutex(execved_shmid);

        // 这边映射不影响后续子进程的映射，不会增加开销
        // inform_aflserver_debug("execved_shmid==-1,1\n");
        void *tmp = shmat(execved_shmid, NULL, 0);
        if(tmp == (void*)-1 || !tmp){
            shmctl(execved_shmid, IPC_RMID, NULL);
        }

        // 在没有找到execved程序的情况下，使用特定格式告知afl-server 产生了新的execved进程
        // 在这里先创建lock，之后在exec之后再去获取就可以?
        // 获取real_path的末位文件名称
        char* real_filename = (char*)malloc(0x10);
        if(strstr(real_path, "/") != NULL){
            strncpy(real_filename, strrchr(real_path, '/') + 1, 0xf);
        }else{
            strncpy(real_filename, real_path, 0xf);
        }
        real_filename[0xf] = '\0'; 
        if(inform_aflserver_exec(file_hash, execved_shmid, real_filename) !=0 ){
            perror("inform_aflserver_exec wrong!\n");
            exit(-1);
        }
        free(real_filename);
        // inform_aflserver_debug_fmt("pid:%d,shmid:%d,exec,M:%s", getpid(),execved_shmid, getenv("AFL_MAIN_BIN"));
    ...
    // fprintf(bk_stdout, "[qemu_exeve, 3] i_name = %s, i_name_dup = %s\n", i_name, i_name_dup);
    

    // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d, reach 2\n", filename ,execved_shmid, getpid());
    ...
    // tokcount是qemu使用execve patch之后的参数以空格分隔的数量
    fprintf(bk_stdout, "[qemu_exeve] reach 2\n");

    // offset之后的是原先程序的参数
    offset += 2 + tokCount + 2; // 加入共享内存地址?都需要加入，不需要判断

    new_argp = alloca((argc + offset + 1) * sizeof(void *));
    // fprintf(bk_stdout, "[qemu_exeve, 4] i_name = %s, i_name_dup = %s\n", i_name, i_name_dup);

    char* shmaddr = alloca(0x10);
    memset(shmaddr, 0, 0x10);
    snprintf(shmaddr, 0x10, "%d", execved_shmid);
    ...
    // fprintf(bk_stdout, "[qemu_exeve] now add -o argument, i_name = %s, offset = %d, argv[0] = %s, i_name_dup = %s\n", i_name, offset, argv[0], i_name_dup);
    if (i_name_dup) {
        new_argp[2] = i_name_dup;
        offset -= 1; // iname is 2nd and 2nd last arg

    } else {
        new_argp[2] = argv[0]; // 一般调用execve时，argv[0]时目的binary名称
    }
    fprintf(bk_stdout, "[qemu_exeve] reach 3 \n");
    // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d, reach 3\n", filename ,execved_shmid, getpid());
    ...
    while (tokCount > 0 && token != NULL) {
        token = strtok(NULL, " ");
        new_argp[offset - 2 - tokCount - 2] = strdup(token);
        tokCount -= 1;
    }

    // printf("offset: %d\n", offset);
    // printf("qemu_execve_path: %s\n", qemu_path_tokens);
    // printf("iname: %s, arg: %s\n", i_name, i_arg);


    new_argp[offset - 2] = strdup("-execve");
    new_argp[offset - 1] = strdup(qemu_execve_path);

    new_argp[offset - 4] = strdup("-shmid");
    
    new_argp[offset - 3] = strdup(shmaddr); // 加入共享内存地址
    // new_argp[offset - 2] = strdup("-hackproc");
    // new_argp[offset - 1] = strdup("-hacksysinfo");
    

    if (i_name_dup) { // bash脚本中才会出现iname
        offset += 1; // iname is 2nd and 2nd last arg
        new_argp[offset - 1] = i_name_dup;

        if (i_arg) {
            new_argp[offset - 2] = i_name_dup;
    ...
    // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d, reach 4\n", filename ,execved_shmid, getpid());

    // debug_print_arg_env(new_argp, envp);
    ...
    for(int i = 0; ; i++){
        ...
        if(strcasestr(arg, "wzq") || strcasestr(arg, "xhy")){
            // 存在命令注入
            inform_aflserver_cmdinj(global_shmid, global_cur_pc);
            FILE *tmp_fp = fopen("cmdlist.txt", "a+");
            fprintf(tmp_fp, "[cmdinj!!] vuln arg: %s\n", arg);
            fclose(tmp_fp);
        }
    }
    // inform_aflserver_debug_fmt("will execve %s\n", qemu_path);
    // for(int i = 0; ; i++){
    //     char* arg = new_argp[i];
    //     if(arg == NULL){
    //         break;
    //     }
    //     inform_aflserver_debug_fmt("arg[%d]: %s\n", i, arg);
    // }

    // 这个环境变量用于避免redqueen记录quemu运行时的字符串，但是好像没有用处
    remove_env_var(envp, "AFL_QEMU_CHILD_SETUP");
    
    // fprintf(bk_stdout, "[qemu_exeve] reach 5 ,qemu_path:%s\n", qemu_path);
    // // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d, reach 5\n", filename ,execved_shmid, getpid());
    for(int i = 0; ; i++){
        char* arg = new_argp[i];
        if(arg == NULL){
            break;
        }
        fprintf(bk_stdout, "[qemu_exeve] arg[%d]: %s\n", i, arg);
    }
    free(real_path);
    // for(int i = 0; ; i++){
    //     char* arg = envp[i];
    ...
    //         break;
    //     }
    //     fprintf(bk_stdout, "[qemu_exeve] env[%d]: %s\n", i, arg);
    // }
    

    // fprintf(exec_fp, "[qemu_execve] %s got execved_shmid %d, pid=%d, reach 6\n", filename ,execved_shmid, getpid());
    // fclose(exec_fp);
    // ret = get_errno(safe_execve(qemu_path, new_argp, envp));
...
static bool wait_for_self_pid(pid_t child_pid){
    double timeout = 0.5;
                            ...
                            abi_long arg8)
{
    ...
    // fprintf(bk_stdout, "%s(%d),syscall:%d\n", gl_argv_0, getpid(), num);
    // inform_aflserver_debug_fmt("pid:%d,syscall:%d", getpid(), num);
    ...
    case TARGET_NR_write:
        ...
        // 下面加入了AFL_MAIN_BIN的判断是因为同级进程中，不能在发送了消息之后就直接退出
        // 但是用write来判断终止感觉不理想，因为固件可能通过fwrite多次写入请求内容
        // if(hookhack && conn_fd != -1 && fd == conn_fd && hookhack_recved) {
    ...
    case TARGET_NR_close:
    {
        ...
        //     fprintf(close_fp, "now close %d, conn_fd is %d, http_fd=%d\n", arg1, conn_fd, http_fd);
        //     fclose(close_fp);
        // }
        // fprintf(bk_stdout, "[HOOK] close fd: %d\n", arg1);
        // if(arg1 == 12){
        //     // don't why for totolink xr5000
        //     return 0;
        // }
        ...
        if(conn_fd >=0 && arg1 == conn_fd) {
            ...
            if(getenv("AFL_MAIN_BIN") && strcmp(getenv("AFL_MAIN_BIN"), "1")==0){
                ...
                // // 这里有一个tricky的条件竞争
                // // 如果主进程先产生子进程，之后关闭了socket(子进程还没开始执行)
                // // 那么现实中，一般主进程还存活，从而能够正常处理子进程
                // // 但是现在直接exit之后，产生的子进程在execve之前将找不到写入的管道，导致SIGPIPE，导致无法执行execve
                // // 解决这个问题的方法是，主进程确实需要exit(为了fuzzing持续运行)，但是exit之前需要检查子进程都已经结束
    ...
    case TARGET_NR_unlink:
        return 0;
        // 不删除文件就行了，因为目标文件只要写入了，都在/afltmp下，会被afl删除。其余的没有写入也不应该删除
        if (!(p = lock_user_string(arg1)))
            return -TARGET_EFAULT;
    ...
    case TARGET_NR_access:
        ...
        ret = get_errno(do_access(path(p), arg2));
        unlock_user(p, arg1, 0);
        return ret;
#endif
    ...
    case TARGET_NR_dup:
    {
        // FILE* fp = fopen("/qemu_dup.log", "a+");
        ret = get_errno(dup(arg1));
        if (ret >= 0) {
            fd_trans_dup(arg1, ret);
        }
        // fprintf(fp, "make dup(%d), got %d\n", arg1, ret);
        // fclose(fp);
        return ret;
...
#define MMAP_SHIFT 12
    ...
    case TARGET_NR_shutdown:
        if(conn_fd >=0 && arg1 == conn_fd) {
            ...
            // FILE* close_fp = fopen("/qemu_close.log", "a+");
            //     for(size_t cnt = 0; cnt < self_child_cnt; cnt++){
            //             fprintf(close_fp, "[HOOK_shutdown] wait for pid=%d\n",self_childs[cnt]);
            //             while(wait_for_self_pid(self_childs[cnt]) == false){
            //             ;
            //         }
            //     }
    ...
    case TARGET_NR_tgkill:
        {
          ...
          if(sig == SIGABRT || sig == SIGSEGV){
            // inform_aflserver_bug(global_shmid, global_cur_pc);
            // FILE* fp = fopen("/buginfo.txt", "a+");
            //     fprintf(fp, "TARGET_SIGSEGV has generated, in syscall.c\n");
            //     fclose(fp);
          }
...
/*         break; */
    ...
    {
        struct epoll_event *ep = (struct epoll_event *)malloc(sizeof(struct epoll_event));
        struct epoll_event *epp = 0;
        if (arg4) {
            if (arg2 != EPOLL_CTL_DEL) {
                // arg2是epoll控制部分，还可能是EPOLL_CTL_ADD以及EPOLL_CTL_MOD
             ...
             */
            epp = ep;
            // 这里应该记录此时socket对应的epoll_event结构体，也就是ep，并在epoll_wait返回,arg3是fd
            fprintf(bk_stdout, "[epoll_ctl] arg3=%d, arg4=%d, ep.events=0x%x\n", arg3, arg4, ep->events);
            fprintf(bk_stdout, "coltrol flag is %d\n", arg2);
            struct stat statbuf;
            struct sockaddr_in sin;
            socklen_t len = sizeof(sin);
            uint16_t port = 0;
            // 检查arg3是不是http 对应端口上的fd
            if(conn_fd == -1){
                if(fstat(arg3, &statbuf)){fprintf(bk_stdout, "[epoll_ctl] fstat");goto do_epoll_ctl;}
                if(!S_ISSOCK(statbuf.st_mode)){fprintf(bk_stdout, "[epoll_ctl] S_ISSOCK");goto do_epoll_ctl;}
                if(getsockname(arg3, (struct sockaddr* )&sin, &len)){fprintf(bk_stdout, "[epoll_ctl] getsockname");goto do_epoll_ctl;}
                port = ntohs(sin.sin_port);
                fprintf(bk_stdout, "[epoll_ctl] listen at port %d\n", port);
                if(port != fake_port){goto do_epoll_ctl;}
                http_fd = arg3;
                epoll_event_addr = ep; // 保存http 80端口对应的event,也就是这里注册的arg4。后面在epoll_wait会直接返回arg4吗
                fprintf(bk_stdout, "[HOOK] epoll_ctl, http_fd: %d\n", http_fd);
            }else{
                // 已经通过epoll_wait获取了一个http fd
                if(arg3 == conn_fd){
                    // 记录下accept对应的event，之后发送给epoll_wait作为返回值
                    epoll_conn_event_addr = ep;
                }
            }
...
#endif
    {
        fprintf(bk_stdout, "[epoll_pwait] doing epoll_pwait\n");
        struct target_epoll_event *target_ep;
        ...
        // 这里也要实现hookhack逻辑
        {
            ...
            ret = do_epoll_pwait(epfd, ep, maxevents, timeout,
                                             set, SIGSET_T_SIZE);
            break;
...
#if defined(TARGET_NR_epoll_wait)
        case TARGET_NR_epoll_wait:
            // 这里greenhouse fuzzer并没有hookhack,导致存在进程可能卡在这
            // fprintf(bk_stdout, "on epoll wait\n");
            ret = do_epoll_wait(epfd, ep, maxevents, timeout);
            // ret = get_errno(safe_epoll_pwait(epfd, ep, maxevents, timeout,
                                            //  NULL, 0));
            break;
#endif
