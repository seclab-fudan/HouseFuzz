...
char *gl_argv_0; // 目标程序名称，使用时需要截断，只获取 "/"最后的内容
...
FILE *bk_stdout;
int fake_port = 0;
char* trace_progname = NULL; // GREENHOUSE PATCH by XHY
...
static void handle_arg_execshmid(const char *arg)
{
    qemu_set_execshmid(arg);
}
...
static void handle_arg_tracename(const char* arg)
{
    trace_progname = strdup(arg);
}
...
static void handle_arg_hacksleep(const char* arg)
{
    qemu_set_hacksleep(arg);
}

static void handle_arg_hackselect(const char* arg)
{
    qemu_set_hackselect(arg);
}
...
static const struct qemu_argument arg_table[] = {
    ...
    {"shmid",      "QEMU_EXECVE_SHMID",true,  handle_arg_execshmid,
     "",           "the shmid for the execved program to use"},
    ...
    {"hacksysinfo","QEMU_SYSINFO",    false,   handle_arg_hacksysinfo,
     "",           "use hack to get around needing to mount a writable /proc"},
    {"hacksleep","QEMU_HACKSLEEP",    false,   handle_arg_hacksleep,
     "",           "use hack to avoid sleep system call to return instantlly"},
    {"tracename",  "QEMU_TRACENAME",   true,    handle_arg_tracename,
    "",             "trace the given prog's pc address "                    },
    {"execve",     "QEMU_EXECVE",      true,   handle_arg_execve,
     "path",       "use interpreter at 'path' when a process calls execve()"},
    {"hackselect", "QEMU_HACKSELECT",  false,  handle_arg_hackselect,
    "",            "hack select for child process to not wait for too long"},
...
static void set_glargv0(char* qemu_argv, char* gl_argv){
    // 主要作用就是获取路径末尾的变量名称
    // char* real_name = check_symlink_me(qemu_argv);
    const char* filename = strrchr(qemu_argv, '/');
    if(filename == NULL) {  // No path separator
        strncpy(gl_argv, qemu_argv, 0x10);
    } else {                // Found a path separator
        strncpy(gl_argv, filename + 1, 0x10);
    }
}
...
static int parse_args(int argc, char **argv)
{
    ...
    // Add by XHY
    char *port_str = getenv("INTERFACE_PORT");
    fake_port = 0;
    if (port_str) {
        fake_port = atoi(port_str);
    }
    if (!fake_port) {
        printf("Warning: INTERFACE_PORT is not set, using default port 80\n");
        fake_port = 80;
    }
...
int main(int argc, char **argv, char **envp)
{
    ...
    // FILE* qemu_initfp = fopen("/qemu_init.log", "a+");
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 2\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 3\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 4\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 5\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 6\n", getpid());
    ...
    // printf("in main, argv[0] is %s\n", target_argv[0]);

    gl_argv_0 = (char*)malloc(0x10);
    memset(gl_argv_0, '\0', 0x10);

    set_glargv0(target_argv[0], gl_argv_0);
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 7\n", getpid());

    // printf("after set_glargv0 , gl_argv_0 is %s\n", gl_argv_0);
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 8\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 9\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 10\n", getpid());
    ...
    syscall_init();
    signal_init();
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 11\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, 12\n", getpid());
    ...
    // fprintf(qemu_initfp, "qemu_init, myself pid is %d, before cpu_loop\n", getpid());
    // fclose(qemu_initfp);
    cpu_loop(env);
    /* never exits */
    return 0;
