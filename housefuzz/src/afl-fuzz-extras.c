...
void merge_json_arrays(json_t* arr1, const json_t* arr2) {
    if (!json_is_array(arr2)) {
        FATAL("merge json_array, one target is null");

    }

    size_t i, size2 = json_array_size(arr2);
    for (i = 0; i < size2; i++) {
        json_t* value = json_array_get(arr2, i);
        json_array_append(arr1, value);
    }
}


void update_main_dict(afl_state_t *afl, char* name){
    // 找到afl->fsrv.target_path在字典afl->all_extras中的对应项
    // 并将这一项加入到afl->json_extras中
    // 同时更新afl->extras_cnt
    void* tmp;
    const char* key;
    json_t* value;
    json_object_foreach_safe(afl->all_extras, tmp, key, value){
      if(strstr(name, key) || strstr(key, name)){
        json_t* combined_item = json_object_get(afl->all_extras, "combined");
        if(json_object_get(combined_item, key) != NULL){
          // 说明已经添加过
          return;
        }

        // 没有添加过，添加到主字典
        if(afl->json_extras == NULL){
          afl->json_extras = json_array();
        }
        merge_json_arrays(afl->json_extras, value);
        afl->extras_json_cnt = json_array_size(afl->json_extras);
        // 写入已经添加过的标志
        json_object_set_new(combined_item, key, json_string("1"));
        break;
      }
    }
...
void load_json_extras(afl_state_t *afl, u8 *fname){
  // 读取json形式的extra文件，并作为key储存在afl->json_extras
  json_error_t error;
  
  if(unlikely(afl->debug)){
    printf("inside load_json_extras, afl->main_procname=%s\n", afl->main_procname);
  }
  json_t* json_dict = json_load_file(fname, 0, &error);
  if(!json_dict){
    FATAL("unable to load json extra file");
  }
  if(!json_is_array(json_dict) && !json_is_object(json_dict)){
    json_decref(json_dict);
    FATAL("not json object");
  }


  afl->all_extras = json_dict;
  json_object_set_new(json_dict, "combined", json_object());
  // afl->json_extras = json_dict;
  // 寻找当前主进程对应的字典
  if(afl->main_procname == NULL || strlen(afl->main_procname) < 2){
    FATAL("wrong -P argumnent (indicate main binary name)");
  }
  update_main_dict(afl, afl->main_procname);

  if(afl->extras_json_cnt == 0){
    FATAL("load_json_extras, extras_json_cnt==0");
  }

  ACTF("totall load json_dict size is %u", afl->extras_json_cnt);
  
}

void load_protocal_dict(afl_state_t *afl, u8 *fname){
  // return ;
  json_error_t error;
  json_t* json_dict = json_load_file(fname, 0, &error);
  ...
  afl->upnp_dict = json_dict;

  size_t upnp_size = json_array_size(json_dict);

  ACTF("upnp directory total size: %lu", upnp_size);
}
...
/* 从api dict中读取uri以及name */

void load_extras_api(afl_state_t* afl, u8* fname){

  ACTF("Loading api dictionary from '%s' ...", fname);

  json_error_t error;
  json_t* api_dict = json_load_file(fname, 0, &error);

  if(!api_dict){
    FATAL("can't load api dict");
  }

  if(!json_is_object(api_dict)){
    json_decref(api_dict);
    FATAL("not json object");
  }


  afl->perfix_extra = json_object_get(api_dict, "perfix");
  afl->perfix_extra_cnt = json_array_size(afl->perfix_extra);

  afl->name_extra = json_object_get(api_dict, "name");
  afl->name_extra_cnt = json_array_size(afl->name_extra);

  afl->fulluri_extra = json_object_get(api_dict, "full");
  afl->fulluri_extra_cnt = json_array_size(afl->fulluri_extra);

  afl->headers_extra = json_object_get(api_dict, "headers");
...
void load_extras(afl_state_t *afl, u8 *dir) {
  ...
  if (!d) {
    ...
    if (errno == ENOTDIR) {
      ...
      if(afl->no_grammar_mode){
          load_extras_file(afl, dir, &min_len, &max_len, dict_level);
          extras_check_and_sort(afl, min_len, max_len, dir);
      }
      else{
        load_json_extras(afl, dir);
      }
      
      // 这里暂时直接写upnp dict为硬编码，后面需要在参数中加上upnp dict
      // load_protocal_dict(afl, "/upnp_dict.json");
      return;
