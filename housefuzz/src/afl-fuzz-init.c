  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    ...
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        afl->queued_imported += save_if_interesting_json(afl, mem, len, fault, NULL);
        afl->syncing_party = 0;
...
void perform_dry_run(afl_state_t *afl) {
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    if(afl->no_grammar_mode){
      ...
      if (root == NULL) { PFATAL("Unable to open '%s'", q->fname); }

      size_t read_len = 0;
      printf("init, before json_to_http, readlen=%lu\n", read_len);
      use_mem = json_to_http(root, &read_len);
      q->len = read_len;
      printf("after json_to_http, readlen=%lu\n", read_len);
    }
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  tmp = alloc_printf("%s/bincov_data", afl->out_dir);
  if (!afl->in_place_resume) {
    ...
    afl->fsrv.bincov_file = fdopen(fd, "w");
    if (!afl->fsrv.bincov_file) { PFATAL("fdopen() failed"); }

    fprintf(
        afl->fsrv.bincov_file,
        "# relative_time, [name, shmid, cov, activated_time]\n");
    ...
    fseek(afl->fsrv.bincov_file, 0, SEEK_END);

  }
  fflush(afl->fsrv.bincov_file);
...
/*当收到SIGUSR2信号时，代表afl-server读取了套接字中的数据，需要告知afl-fuzz*/
/*这里是fuzzer的处理方法*/
// static void handle_server_pipe(int sig){
//   (void)sig;
//   afl_states_handle_pipe();
// }
...
void init_domain_socket(afl_state_t *afl){
    afl->domain_fd = socket(PF_UNIX, SOCK_STREAM, 0);
    if(afl->domain_fd < 0){
          FATAL("socket() failed");
        }
    unlink(SOCK_NAME);

    afl->domain_server.sun_family = AF_UNIX;
    strncpy(afl->domain_server.sun_path, SOCK_NAME, sizeof(afl->domain_server.sun_path)-1);
    if(bind(afl->domain_fd, (struct sockaddr *)&afl->domain_server, sizeof(struct sockaddr_un)) != 0){
      FATAL("bind() failed");
    }

    if(listen(afl->domain_fd, 10) != 0){
      FATAL("listen() failed");
      printf("errno: %s\n", strerror(errno));
    }
...
int sendto_aflfuzz(afl_state_t *afl, char* buf, size_t size){
  
  pthread_mutex_lock(mutex);
  int result = write(afl->domain_child2parent[1], buf, size);
  // pthread_mutex_unlock(mutex);
  

  if(result < 0){
    printf("error in sendto_aflfuzz\n");
  }
  // 向afl->fuzzer发送信号

  int kill_result = kill(afl->fuzzer_pid, SIGUSR2);
  if(kill_result == 0){
  ...
  return (result && kill_result);
}


/*使用select()模型循环等待多进程的连接*/
...
void wait_connect(afl_state_t *afl){
  ...
  int ret = 0;
  int result = 0;
  int max_sock = afl->domain_fd; // max_sock是select监听的文件描述符的上限
  char buf[0x30] = {0};
  char reply_buf[0x20] = {0};
  // FILE* fp = fopen("/aflserver.log", "a+");

  memset(fd, '\0', sizeof(int)*30);
  memset(buf, '\0', 0x30);
  ...
  while(1){
    ...
    for(int i = 0; i < max_client; i++){
      if(FD_ISSET(fd[i], &fdsr)){
        ...
        if(ret <= 0){
          ...
          // 检查buf空间大小是否足够,下边的buf就是用户发来的数据
          if(ret < 0x30){
            memset(&buf[ret], '\0', 1);

            // printf("afl-server: buf is %s\n", buf);
            // 通过pipe传输给父进程
            // **是否可以直接传输shmid给父进程从而监视共享内存?**
            // 现在直接传输buf
            // fprintf(fp, "%s\n", buf);
            if(sendto_aflfuzz(afl, buf, ret+1) != 0){
              FATAL("afl-fuzz pipe is truncated!");
            }

            if(buf[0] == 'R' || buf[0] == 'Q'){
              memset(reply_buf, '\0', 0x20);
              // 请求报文，需要等待父进程回应并写入子进程
              result = read(afl->domain_parent2child[0], reply_buf, 0x20);
              if(result > 0){
                  send(fd[i], reply_buf, 0x20, MSG_DONTWAIT);
              }
            }
          }
        }
    ...
    if(FD_ISSET(afl->domain_fd, &fdsr)){
      int new_fd = accept(afl->domain_fd, (struct sockaddr *)&client_addr, &addrlen);
      if(new_fd < 0){
        printf("%s\n", strerror(errno));
        FATAL("ACCEPT ERROR with result %d\n", new_fd);
        continue;
      }
...
//   /*处理pipe事件产生。handle_server_pipe为server在收到一个连接请求之后，将连接中数据传输给afl-fuzz之后，afl-fuzz的处理函数*/
//   sa.sa_handler = handle_server_pipe;
