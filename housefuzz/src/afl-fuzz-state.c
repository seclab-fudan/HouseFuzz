...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->no_grammar_mode = 0;
  afl->favored_seed_type = SEED_TYPE_JSON;
#ifndef NO_SPLICING
  afl->use_splicing = 1;
#endif
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  for(u32 cnt = 0; cnt < MAX_PROC_CNT; cnt++){
    if(afl->proc_list[cnt] != NULL){
      ck_free(afl->proc_list[cnt]->virgin_bits);
      shmdt(afl->proc_list[cnt]->trace_bits);
      free(afl->proc_list[cnt]);
    }
  }
