...
inline u32 select_next_json_entry(afl_state_t *afl) {
   if(afl->json_queue_cnt == 0){
    FATAL("unknown why afl->json_queue_cnt == 0");
   }
   int rand_num = rand() % afl->json_queue_cnt;
   return afl->json_queue[rand_num];

}

/* try to enforce queue seed type with multiple attempts */

inline u32 select_next_queue_entry_adapt(afl_state_t *afl, u32 max_tries) {

  if (afl->queued_items <= 1) return 0;
  u32 s;

  if (afl->favored_seed_type == SEED_TYPE_JSON) {
    return select_next_json_entry(afl);
  }else{
      do {
      s = select_next_queue_entry(afl);
      struct queue_entry *q = afl->queue_buf[s];
      if (q->seed_type == afl->favored_seed_type) {
        return s;
      }
    } while (--max_tries > 0);
  }
...
double compute_weight(afl_state_t *afl, struct queue_entry *q,
  ...
  // to prioritize the possiblility of choosing json file, multiple it by 5 times
  if(q->seed_type == SEED_TYPE_JSON){
    weight *= 5;
  }
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  if (afl->no_grammar_mode)
    q->seed_type = SEED_TYPE_RAW;
  else{
    if(!fname || strstr(fname, ".raw") != NULL){
      q->seed_type = SEED_TYPE_RAW;
    }else{
      q->seed_type = SEED_TYPE_JSON;
    }
  }
  ...
  if(q->seed_type == SEED_TYPE_JSON){
    if(likely(afl->json_queue_cnt < afl->json_queue_maxsize - 1)){
      afl->json_queue[afl->json_queue_cnt++] = q->id;
    }else{
      afl->json_queue = realloc(afl->json_queue, 2*afl->json_queue_maxsize);
      afl->json_queue_maxsize *=2;
      afl->json_queue[afl->json_queue_cnt++] = q->id;
    }
  }
  ...
  for (i = 0; i < afl->fsrv.map_size; ++i) {
    ...
    if (afl->fsrv.trace_bits[i]) {
      ...
      if (!q->trace_mini) {
        ...
        minimize_bits(afl, q->trace_mini, afl->fsrv.trace_bits); // 8 bit -> 1 bit
...
u32 calculate_score_ori(afl_state_t *afl, struct queue_entry *q) {
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if(q->seed_type == SEED_TYPE_RAW){
    return calculate_score_ori(afl, q);
  }
  ...
  u32 avg_bitmap_size = afl->total_bitmap_size / afl->total_bitmap_entries;
  u32 perf_score = 100;
  // FILE* score_fp = fopen("score.txt", "a+");
  // fprintf(score_fp, "stage 0, perf_score=%d\n", perf_score);
  ...
  for(u32 cnt = 0; cnt < MAX_PROC_CNT; cnt++){
    ...
    ipc_cnt += afl->proc_list[cnt]->activated;
  ...
  // fprintf(score_fp, "stage 6, perf_score=%d\n", perf_score);
...
inline u8 *queue_testcase_get(afl_state_t *afl, struct queue_entry *q) {
  ...
  if(q->seed_type == SEED_TYPE_JSON){
    ...
    size_t tmp_size = 0;
    char* testcase_buf_ptr = NULL;
    testcase_buf_ptr = json_to_http(queue_testcase_get_json(afl, q), &tmp_size);
    
    if(testcase_buf_ptr == NULL){
      FATAL("testcase_buf_ptr == NULL in queue_testcase_get");
    }

    q->testcase_buf = testcase_buf_ptr;
  }
...
json_t* load_json_packet(char* pathname){
    json_t *root;
    json_error_t error;
    root = json_load_file(pathname, 0, &error);
    if(!root){
...
json_t *queue_testcase_get_json(afl_state_t *afl, struct queue_entry *q) {
  ...
  /* first handle if no testcase cache is configured */
  // q_testcase_max_cache_size 在afl-fuzz-state.c中初始化
  json_t *root = NULL;
  if (unlikely(!afl->q_testcase_max_cache_size)) {
    ...
    root = load_json_packet(q->fname);

    if (unlikely(root == NULL)) { PFATAL("Unable to load '%s'", q->fname); }
    q->root = root;
  ...
  if (unlikely(!q->root)) {
    ...
    while (unlikely(
      ...
      struct queue_entry *old_cached = afl->q_testcase_cache[tid];
      json_object_clear(old_cached->root);
      
      old_cached->root = NULL;
      free(old_cached->testcase_buf);
      old_cached->testcase_buf = NULL;
    ...
    if (unlikely(root == NULL)) { PFATAL("Unable to open or load '%s'", q->fname); }
...
inline void queue_testcase_store_mem_json(afl_state_t *afl, struct queue_entry *q,
                                     json_t* input_json) {
  ...
  q->root = json_deep_copy(input_json);
