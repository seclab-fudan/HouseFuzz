...
void maybe_update_plot_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  int write_bincov_file = 0;
  fprintf(afl->fsrv.bincov_file,
          "%llu\n", (afl->prev_run_time + get_cur_time() - afl->start_time) / 1000);
  for(u32 cnt = 0; cnt < MAX_PROC_CNT; cnt++){
    ...
    write_bincov_file = 1;
    fprintf(afl->fsrv.bincov_file, "[%s,%d,%d,%d]", afl->proc_list[cnt]->prog_name, afl->proc_list[cnt]->shmid, afl->proc_list[cnt]->edges_found, afl->proc_list[cnt]->activated_times);
  }
  if(write_bincov_file){
    fprintf(afl->fsrv.bincov_file, "\n");
  }
...
void show_stats(afl_state_t *afl) {
  ...
  u64 cur_ms;
  u32 t_bytes, t_bits;
  u32 t_bytes_ipc[MAX_PROC_CNT];
  double t_bytes_ipc_ratio[MAX_PROC_CNT];
...
#define IB(i) (val_buf[(i)])
  ...
  memset(t_bytes_ipc, 0, sizeof(u32)*MAX_PROC_CNT);
  ...
  for(u32 cnt = 0; cnt < MAX_PROC_CNT; cnt++){
    if(afl->proc_list[cnt] != NULL){
      ...
      // fprintf(fp_stats, "afl->proc_list[%d]->activated=%d\n", cnt, afl->proc_list[cnt], afl->proc_list[cnt]->activated);
          t_bytes_ipc[cnt] = proclist_count_non_255_bytes(afl->proc_list[cnt]->virgin_bits);
          // fprintf(fp_stats, "[show_stats_normal] t_bytes_ipc[%d] is %d\n", cnt, t_bytes_ipc[cnt]);
          t_bytes_ipc_ratio[cnt] = ((double)t_bytes_ipc[cnt]*100) / MAP_SIZE;

          // update
          afl->proc_list[cnt]->edges_found = t_bytes_ipc[cnt];
    }else{
      break;
    }
  }
...
#define SP20 SP10 SP10
  ...
  char *cmdinj_color = afl->total_cmdinj   ? cLRD
                      : afl->saved_cmdinj  ? cYEL
                                           : cRST; 
  ...
  SAYF(bVR bH bSTOP                                             cCYA
       " ipc found" bSTG bH10 bH5 bH2 bH2 bH5 bH2 bH bX bH bH bSTOP cCYA
       " coverage " bSTG bH10 bH5 bH10                  bVL "\n");

  sprintf(tmp, "%-5d (%0.02f%%)",  t_bytes , t_byte_ratio);
  SAYF(bV bSTOP "  IPC shmid: " cRST "%-24s " bSTG bV bSTOP
                " coverage : " cRST "%-25s" bSTG   bV  bSTOP"\n",
          "main", tmp);

  for (u32 _cnt = 0; _cnt < MAX_PROC_CNT; _cnt++){
    if(afl->proc_list[_cnt] == NULL){
      break;
    }
    if(proc_in_blacklist(afl, afl->proc_list[_cnt])){
          continue;
    }
    sprintf(tmp, "%-5d (%0.02f%%)",  t_bytes_ipc[_cnt] , t_bytes_ipc_ratio[_cnt]);

    SAYF(bV bSTOP "  IPC shmid: " cRST "%-15s(%-8d)" bSTG bV bSTOP
                " coverage : " cRST "%-14s" "time:%-6d" bSTG bV bSTOP "\n",
          afl->proc_list[_cnt]->prog_name, afl->proc_list[_cnt]->shmid, tmp, afl->proc_list[_cnt]->activated_times);
  ...
  sprintf(tmp, "%s (%lld saved)", u_stringify_int(IB(0), afl->total_cmdinj), afl->saved_cmdinj);

  /* Yeah... it's still going on... halp? */

  SAYF(bV bSTOP "  now trying : " cRST "%-22s " bSTG bV bSTOP
                " cmdinj founds : " cRST "%s%-20s" bSTG   bV "\n",
       afl->stage_name, cmdinj_color, tmp);
