...
void remove_files_in_directory(const char *path) {
    ...
    while ((dir_entry = readdir(d)) != NULL) {
        // Do not delete the current directory (.) or parent directory (..)
...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout) {
  ...
  for(u32 i = 0; i<MAX_PROC_CNT; i++){
    if(afl->proc_list[i] != NULL && afl->proc_list[i]->activated == 1){
      afl->proc_list[i]->activated = 0;
      afl->proc_list[i]->classified = 0;
      
      memset(afl->proc_list[i]->trace_bits, 0, MAP_SIZE+16);
    }else if(afl->proc_list[i] == NULL){
      break;
    }
  }
  afl->utility_timeout = 0;

  memset(afl->fsrv.main_child, 0, MAX_PROC_CNT*sizeof(u32)); 
  afl->fsrv.main_child_cnt = 0;

  nvram_cow_reload();

  clear_bug_status(afl);

  remove_files_in_directory("/afltmp");
  

  fsrv->domain_fd = afl->domain_fd;
...
// printf("got len write_to_testcase %d\n", len);
...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    u64 cksum;
    // printf("after afl_fsrv_start, 4\n");

    (void)write_to_testcase(afl, (void **)&use_mem, q->len, 1);
    // FATAL("check seed");

    // printf("after afl_fsrv_start, 5\n");
    ...
    for(u32 cnt = 0; cnt < MAX_PROC_CNT; cnt++){
      ...
      if(afl->proc_list[cnt]->activated == 1){
        proclist_classify_counts(afl->proc_list[cnt]->trace_bits);
      }
    }
  ...
  q->bitmap_size += proclist_count_bytes(afl);

  q->ipc_distance = aflmulti_count_ipc_distance(afl);

  q->handicap = handicap;
  q->cal_failed = 0;

  afl->total_bitmap_size += q->bitmap_size;
  afl->total_ipc_distance += q->ipc_distance;
...
void lock_all_mutex(afl_state_t *afl){
 
  for(u32 i = 0; i< MAX_PROC_CNT; i++){
    if(afl->proc_list[i] == NULL){
      break;
    }
    if(afl->proc_list[i]->mutex == NULL){
      continue;
    }
    pthread_mutex_lock(afl->proc_list[i]->mutex);
  }
}
...
void unlock_all_mutex(afl_state_t *afl){
  ...
  for(u32 i = 0; i< MAX_PROC_CNT; i++){
    ...
    pthread_mutex_unlock(afl->proc_list[i]->mutex);
  }
}
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  afl->queued_discovered += save_if_interesting_json(afl, out_buf, len, fault, NULL);
...
u8 __attribute__((hot))
common_fuzz_stuff_json(afl_state_t *afl, u8 *out_buf, u32 len, json_t* json_input) {

  
  u8 ret = 0;
  u8 fault;
  // printf("[common_fuzz_stuff_json, 1] now stage: %d, total=%d\n", afl->stage_cur, afl->stage_max);
  ...
  afl->queued_discovered += save_if_interesting_json(afl, out_buf, len, fault, json_input);
