...
/* 标志是否是afl初始状态，在接收到END消息之后，设置afl_init_proc = 0 */
// global int afl_init_proc = 1;
...
pthread_mutex_t* get_named_shared_mutex(int shmid){
  ...
  int fd = open(filename, O_RDWR);
    if (fd == -1) {
        // handle error
        printf("can't find mutex %s\n", filename);
        FATAL("can't find mutex!");
    }
...
void get_pm_from_string(const char* str, int* pid, int* shmid, char* name) {
...
void proclist_add_back(afl_state_t *afl, int pid, int shmid, char* name){
  ...
  new_proc->valid = 1;
  new_proc->ppid = 0;
  new_proc->fork_addr = 0;
  new_proc->activated = 0;
  new_proc->type = TYPE_BG;
  new_proc->mutex = get_named_shared_mutex(shmid);

  strncpy(new_proc->prog_name, name, 0x10);
...
void init_get_pad_from_string(const char* str, int* ppid, char* prog_name){
    size_t len = strlen(str) + 1;
    char* str_copy = (char*)alloca(len);
    memcpy(str_copy, str, len);
...
int init_proclist_search_shmid(afl_state_t *afl, char* prog_name, int* ret_entry){
  u32 entry;
  for(entry = 0; entry < MAX_PROC_CNT; entry++){
    if(afl->proc_list[entry] == NULL)
      break;
...
void init_get_pmpad_from_string(const char* str, int* pid, int* shmid, char* prog_name){
  size_t len = strlen(str) + 1;
  char* str_copy = (char*)alloca(len);
  memcpy(str_copy, str, len);
...
void init_proclist_setvalid(afl_state_t *afl, u32 entry_index, int pid){
...
struct proc_entry* init_proclist_add_child(afl_state_t *afl, int pid, int shmid, char* prog_name){
  struct proc_entry* new_proc = new_proc_struct();
...
void init_get_pgid_from_string(const char* str, unsigned long* pgid, int *pid){
...
void init_get_pgshmid_from_string(const char* str, unsigned long* pgid, int* shmid, char* name)
{
    // char *str_copy = strdup(str);
    size_t len = strlen(str) + 1;
    char* str_copy = (char*)alloca(len);
...
void init_proclist_add_exec(afl_state_t *afl, unsigned long pgid, int shmid, char* name){
  struct proc_entry* new_proc = new_proc_struct();
  new_proc->shmid = shmid;
  new_proc->valid = 1;
  new_proc->activated = 1;
...
int init_proclist_search_pgid(afl_state_t *afl, unsigned long pgid, u32* ret_entry){
  u32 entry;
  for(entry = 0; entry < MAX_PROC_CNT; entry++){
...
void read_until_sendok(afl_state_t* afl, int domain_fd){
  char buffer[0x30];
  int pid = 0;
  int shmid = 0;
  char prog_name[0x10];
  ...
  int result = 0;
  while(1){
    printf("wait for initial socket\n");
    memset(buffer, 0, 0x30);
    client_sock = accept(domain_fd, (struct sockaddr *)&client, &client_addr_len);
    int bytes_read = recv(client_sock, buffer, 0x30, 0);
    if(bytes_read == -1){
      FATAL("afl sock error");
    }
    if(bytes_read == 0){
      continue;
    }
    ...
    if(buffer[0] == 'O'){
      ...
      get_pm_from_string(buffer, &pid, &shmid, prog_name);
      proclist_add_back(afl, pid, shmid, prog_name);
      continue;
    }else{
      if(buffer[0] == 'R')
      {
        init_get_pad_from_string(buffer, &pid, prog_name);
        
        shmid = init_proclist_search_shmid(afl, prog_name, &entry_index);
        
        
        memset(msg_buf, '\0', 0x20);
        sprintf(msg_buf, "%d", shmid);
        result = write(client_sock, msg_buf, 0x20);
        if(shmid != -1){
          init_proclist_setvalid(afl, entry_index, pid);
        }
        
        if(result < 0){
          FATAL("afl-server sendback error");
        }
        continue;
      }
    
      else if(buffer[0] == 'C'){
      init_get_pmpad_from_string(buffer, &pid, &shmid, prog_name);
      init_proclist_add_child(afl, pid, shmid, prog_name);
      continue;
      }
      else if(buffer[0] == 'Q'){
        init_get_pgid_from_string(buffer, &program_id, &pid);
        int shmid = init_proclist_search_pgid(afl, program_id, &entry_index);
        if(shmid != -1){
              afl->proc_list[entry_index]->activated = 1;
              afl->proc_list[entry_index]->valid = 1;
              afl->proc_list[entry_index]->activated_times += 1;
        ...
        init_get_pgshmid_from_string(buffer, &program_id, &shmid, prog_name);
        init_proclist_add_exec(afl, program_id, shmid, prog_name);
      }
    }
  }
}
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Aa:b:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:P:RrQs:S:t:T:uUV:WXx:YZ")) >
      0) {
    ...
    switch (opt) {
      ...
      case 'g':
        ...
        break;
      case 'r':
        afl->no_grammar_mode = 1;
        afl->favored_seed_type = SEED_TYPE_RAW;
        break;
      ...
      case 'a' : {
        if (optarg == NULL) { FATAL("No valid api dict provided. Got NULL."); }
        load_extras_api(afl, optarg);
        break;
      }
      case 'u': {
          init_domain_socket(afl);
          afl->proc_list = ck_alloc(MAX_PROC_CNT * sizeof(u64));

          read_until_sendok(afl, afl->domain_fd);
          srand((unsigned int)time(NULL));
          my_global_afl = afl;

          
          afl->blacklist_proc_cnt = 0;
          afl->blacklist_proc = malloc(afl->blacklist_proc_cnt * sizeof(char *));


          afl->bg_crash = (struct bg_crash_info*)calloc(1, sizeof(struct bg_crash_info));
          if (unlikely(afl->bg_crash == NULL)) {
              FATAL("Failed to allocate memory for bg_crash_info");
          }
          

          printf("before load_guidance_file\n");
          
          load_guidance_file();

          printf("after load_guidance_file\n");

          printf("before nvram_init_all\n");
          nvram_init_all();

          char* nvram_filename = "/all_nvram.ini";
          nvram_load(nvram_filename);
          printf("after nvram_init and load\n");
      ...
      case 'P':{
        afl->main_procname = strdup(optarg);
        break;
      }
  ...
  if (optind == argc || !afl->in_dir || !afl->out_dir || show_help) {
    ...
    if(delete_sockfile() != 0){
      printf("delete sock error\n");
    }
  ...
  afl->json_queue_maxsize = 1024;
  afl->json_queue = (u32*)malloc(afl->json_queue_maxsize*sizeof(int));
  afl->json_queue_cnt = 0;
  if(afl->json_queue == NULL){
    FATAL("cannot init json_queue");
  }
  ...
  afl->redqueen_activated_times = 0;
  afl->redqueen_test = 1;
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    if (map_size <= DEFAULT_SHMEM_SIZE) {
      ...
      afl->fsrv.map_size = DEFAULT_SHMEM_SIZE;
      char vbuf[16];
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      if (likely(!afl->afl_env.afl_pizza_mode)) {
        ...
        if (unlikely(curdate->tm_mon == 20 && curdate->tm_mday == 1)) {
    ...
    do {
      ...
      if (likely(!afl->old_seed_selection)) {
        ...
        afl->current_entry = select_next_queue_entry_adapt(afl, 10);
        afl->queue_cur = afl->queue_buf[afl->current_entry];
