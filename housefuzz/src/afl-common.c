...
char **get_qemu_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  char **new_argv = ck_alloc(sizeof(char *) * (argc + 100));
  if (unlikely(!new_argv)) { FATAL("Illegal amount of arguments specified"); }

  char *qemu_path = find_afl_binary(own_loc, "afl-qemu-trace");
  // char *execve_arg = NULL;
  // char* execve_arg = (char*)malloc(0x100);
  // char execve_arg[0x60];
  // memset(execve_arg, 0, 0x100);
  // 下面在真实的固件中需要修改
  // int ret = asprintf(&execve_arg, "%s -hackbind -hackproc", "/qemu-static");
  // snprintf(execve_arg, 0x100, "%s -hackbind -hackproc -hacksysinfo -E LD_PRELOAD=libnvram_fuzz.so -E AFL_QEMU_CHILD_SETUP=1", "/fuzz_bins/bin/afl-qemu-trace");
  // snprintf(execve_arg, 0xa0, "%s -hackbind -hackproc -E LD_PRELOAD=libnvram_fuzz.so -E AFL_QEMU_CHILD_SETUP=1", "/tmp/test/test_fuzz/afl-qemu-trace");
  // if (unlikely(ret <= 0)) {
    // FATAL("Fail to allocate memory for execve_arg");
  // }
  // printf("qemu_path is %s\n", qemu_path);

  int i = 0;
  new_argv[i++] = qemu_path;
  new_argv[i++] = "-hackbind";
  new_argv[i++] = "-hackproc";
  new_argv[i++] = "-hookhack";
  new_argv[i++] = "-hacksysinfo";
  new_argv[i++] = "-E";
  new_argv[i++] = "LD_PRELOAD=libnvram_fuzz.so";
  new_argv[i++] = "-execve";
  new_argv[i++] = "/fuzz_bins/bin/afl-qemu-trace -hackbind -hackproc -hacksysinfo -E LD_PRELOAD=libnvram_fuzz.so -E AFL_QEMU_CHILD_SETUP=1";
  // new_argv[i++] = "/tmp/test/test_fuzz/afl-qemu-trace -hackbind -hackproc -hacksysinfo -E LD_PRELOAD=libnvram_fuzz.so -E AFL_QEMU_CHILD_SETUP=1";
  char * extra_args = getenv("AFL_QEMU_EXTRA_ARGS");
  if (extra_args) {
    extra_args = strdup(extra_args);
    if (unlikely(!extra_args)) {
      FATAL("Fail to allocate memory for extra_args");
    }

    int execve_arg_idx = i - 1;
    int orig_execve_arg_len = strlen(new_argv[execve_arg_idx]);
    char *execve_arg = (char *)calloc(orig_execve_arg_len + strlen(extra_args) + 8, 1);
    if (unlikely(!execve_arg)) {
      FATAL("Fail to allocate memory for extra_args");
    }
    strcpy(execve_arg, new_argv[execve_arg_idx]);

    char *token = strtok(extra_args, " ");
    int tmp_idx = orig_execve_arg_len;
    while (token && i < 50) {
      int token_len = strlen(token);
      new_argv[i++] = token;
      execve_arg[tmp_idx++] = ' ';
      strcpy(execve_arg + tmp_idx, token);
      tmp_idx += token_len;
      token = strtok(NULL, " ");
    }
    new_argv[execve_arg_idx] = execve_arg;
  }
  new_argv[i++] = "--";
  new_argv[i++] = *target_path_p;
  memcpy(&new_argv[i], &argv[1], (int)(sizeof(char *)) * (argc - 1));
